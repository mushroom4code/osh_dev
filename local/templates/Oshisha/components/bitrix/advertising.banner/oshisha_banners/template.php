...
<div id="carousel-<?=$arResult['ID']?>" class="carousel <?=$arParams['BS_EFFECT']?><?=$arParams['BS_HIDE_FOR_TABLETS']?>" data-interval="<?=$arParams['BS_INTERVAL']?>" data-wrap="<?=$arParams['BS_WRAP']?>" data-pause="<?=$arParams['BS_PAUSE']?>" data-keyboard="<?=$arParams['BS_KEYBOARD']?>" data-ride="carousel">
...
<?}?>	
	...
	<?if($arParams['BS_ARROW_NAV'] == 'Y' || $arParams['PREVIEW'] == 'Y'):?>
		<a href="#carousel-<?=$arResult['ID']?>" class="carousel-nav-<?=$arParams['TYPE']?>-prev carousel_custom" role="button" data-slide="prev">
			<span class="carousel_elem_custom" aria-hidden="true"><i class="fa fa-angle-left" aria-hidden="true"></i></span>
		</a>
		<a href="#carousel-<?=$arResult['ID']?>" class="carousel-nav-<?=$arParams['TYPE']?>-next carousel_custom_next" role="button" data-slide="next">
			<span class="carousel_elem_custom" aria-hidden="true"><i class="fa fa-angle-right" aria-hidden="true"></i></span>
		</a>
	<?endif;?>
	<!--endregion-->
    ...
    <?if($arParams['BS_BULLET_NAV'] == 'Y' || $arParams['BS_PREVIEW'] == 'Y'):?>
        <div class="carousel-indicators">
            ...
            <?while($i < count($arResult['BANNERS'])):?>
                <span data-target="#carousel-<?=$arResult['ID']?>" data-slide-to="<?=$i?>"  <?if($i==0) echo 'class="active"';$i++?>></span>
            <?endwhile;?>
        </div>
    <?endif;?>
	<?}?>
