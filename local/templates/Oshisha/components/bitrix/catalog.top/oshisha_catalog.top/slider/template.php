...
?>
11
    <div class="mb-4 catalog-top bx-<?= $arParams['TEMPLATE_THEME'] ?>" data-entity="<?= $containerName ?>"
         id="<?= $obName ?>">
        <?
        if (!empty($arResult['RAW_ITEMS']) && !empty($arResult['ITEM_ROWS'])) {
            ...
            foreach ($arResult['ITEM_ROWS'] as $key => $rowData) {
                ...
                $rowIds[] = $rowId;
                ?>
                <div class="row catalog-top-slide <?= $activeClass ?>" id="<?= $rowId ?>" data-entity="items-row">
                    <?
                    $rowData['VARIANT'] = 3;
                    switch ($rowData['VARIANT']) {
                                ...
                                $APPLICATION->IncludeComponent(
                                    ...
                                    array(
                                        'RESULT' => array(
                                            ...
                                            'BIG_BUTTONS' => 'N',
                                            'SCALABLE' => 'N',
                                            'ACTUAL_BASKET'=>  $arResult['AR_BASKET'],
                                        ),
                        ...
                        case 1:
                            foreach ($rowItems as $item) {
                                ...
                                <div class="col-6 product-item-big-card">
                                    <? $APPLICATION->IncludeComponent('bitrix:catalog.item', 'oshisha_catalog.item', array(
                                        'RESULT' => array(
                                            'ITEM' => $item,
                                            'AREA_ID' => $areaIds[$item['ID']],
                                            'TYPE' => $rowData['TYPE'],
                        ...
                        case 6:
                            foreach ($rowItems as $item) {
                                ?>
                                <div class="col-6 col-sm-3 col-md-2 product-item-small-card">
                                    <? $APPLICATION->IncludeComponent('bitrix:catalog.item', 'oshisha_catalog.item', array(
                                        'RESULT' => array(
                                            'ITEM' => $item,
                                            'AREA_ID' => $areaIds[$item['ID']],
