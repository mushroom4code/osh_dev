...
(function () {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function (parameters) {
            ...
            this.mobileTotalBlockNode = BX(parameters.totalBlockId);
        ...
        //     var informer = this.mainErrorsNode.querySelector('[data-type="informer"]'), scrollTop, informerPos;
        //
        //     if (informer) {
        //       if(BX.hasClass(informer, 'alert')) {
        ...
        showDeliveryItemsPage: function (page) {
            ...
            let new_box = BX.create('DIV', {props: {className: 'bx-soa-pp-company-parent'}});
            deliveryItemsContainer.append(new_box);
            ...
            for (k = 0; k < this.deliveryPagination.currentPage.length; k++) {
                ...
                //enterego group delivery block

                if (this.deliveryPagination.currentPage[k].PARENT_ID === 'ALL_DELIVERY_GROUP') {
                    new_box.appendChild(deliveryItemNode);
                } else {
        ...
        click_edit: function () {
            let select_block = this.propsBlockNode.querySelector('.bx-soa-section-title-container');
        ...
        getSelectedPersonType: function () {
            var personTypeInput, currentPersonType, personTypeId, i, personLength,
                personTypeLength = this.result.PERSON_TYPE.length;
            if (personTypeLength !== undefined && personTypeLength !== null) {
                personLength = personTypeLength;
            } else {
                personLength = Object.keys(this.result.PERSON_TYPE).length
            }
        ...
        getPersonTypeControl: function (node) {
            ...
            var personTypesCount = this.result.PERSON_TYPE.length,
                currentType, oldPersonTypeId, i, options = [], delimiter = false;

            if (personTypesCount > 2 && this.result.IS_AUTHORIZED) {
                for (i in this.result.PERSON_TYPE) {
                ...
                this.regionBlockNotEmpty = true;
            } else if (personTypesCount === 2 && this.result.IS_AUTHORIZED) {
                for (i in this.result.PERSON_TYPE) {
                    let text;
                    currentType = this.result.PERSON_TYPE[i];
                    if (currentType.CODE === 'FIZ') {
                        text = 'Розничный покупатель';
                    } else if (currentType.CODE === 'URIC') {
                        text = 'Компания';
                    }
                    let inputContainer = BX.create("div", {
                        attrs: {className: "mr-4 d-flex align-items-center"},
                        children: [
                            BX.create('INPUT', {
                                attrs: {
                            ...
                            BX.create('LABEL', {
                                attrs: {
                                    className: "form-check-label ml-2 font_weight_500",
                ...
                for (i in this.result.PERSON_TYPE)
                    if (this.result.PERSON_TYPE.hasOwnProperty(i) && this.result.PERSON_TYPE[i].ID !== '2')
                        node.appendChild(BX.create('INPUT', {
                            props: {
                                type: 'hidden',
                                name: 'PERSON_TYPE',
        ...
        editPaySystemItems: function (paySystemNode) {
            ...
            var paySystemItemsContainer = BX.create('DIV', {props: {className: 'order-md-1 order-2 bx-soa-pp-item-container'}}),
        ...
        createPaySystemItem: function (item) {
            ...
            label = BX.create('DIV', {
                props: {className: 'bx-soa-pp-company-graf-container pay_system height_100'},
                children: [
                    BX.create('INPUT', {
                        props: {
                            ...
                            name: 'PAY_SYSTEM_ID',
                            type: 'checkbox',
                            className: 'bx-soa-pp-company-checkbox hide_input',
                            value: paySystemId,
                            checked: checked
                        }
                    }),
        ...
        editDeliveryInfo: function (deliveryNode, that) {
            if (!this.result.DELIVERY)
                return;
            ...
            child_address = document.querySelector('.none_block');
            ...
            BX.prepend(child_address);
            let elems = document.querySelector('.bx-soa-customer > .none_block');
            BX.remove(elems);
        ...
        selectDelivery: function (event) {
            ...
            this.activeSection = BX.findParent(event.target, {class: "bx-soa-section"});
            var target = event.target || event.srcElement,
                actionSection = BX.hasClass(target, 'bx-soa-pp-company') ? target : BX.findParent(target, {className: 'bx-soa-pp-company'}),
                selectedSection = this.deliveryBlockNode.querySelector('.bx-selected'),
                actionInput, selectedInput, selected;
            selected = this.deliveryBlockNode.querySelector('input[type=checkbox]');
        ...
        editActivePropsBlock: function (activeNodeMode) {
            ...
            var node = activeNodeMode ? this.propsBlockNode : this.propsHiddenBlockNode,
                propsContent, propsNode, selectedDelivery, showPropMap = false, i, validationErrors, newBlock,
                TypePersonBlock;
            newBlock = this.newBlockId.querySelector('#new_block_with_comment_box');
            TypePersonBlock = this.propsBlockNode.querySelector('.mr-4');
            ...
            while (group = groupIterator()) {
                ...
                while (property = propsIterator()) {
                    if (propsNode.classList.contains('delivery')) {
                        if (property.getGroupId() === '2' || property.getGroupId() === '4' ||
                            property.getGroupId() === '6' || property.getGroupId() === '7') {
                            if (this.deliveryLocationInfo.loc === property.getId()
                                || this.deliveryLocationInfo.zip === property.getId()
                                || this.deliveryLocationInfo.city === property.getId() || property.getName() === 'company'
                                || property.getName() === 'contragent')
                                continue;
                            this.getPropertyRowNode(property, propsItemsContainer, false);
                        }
                        continue;
                    } else {
                        if (property.getGroupId() === '1' || property.getGroupId() === '5') {
                            if (this.deliveryLocationInfo.loc === property.getId()
                                || this.deliveryLocationInfo.zip === property.getId()
        ...
        alterProperty: function (property, propContainer) {
            var divs = BX.findChildren(propContainer, {tagName: 'DIV'}),
                ...
                fileInputs, accepts, fileTitles, option;
            ...
            if (textNode) {
                ...
                textNode.setAttribute('data-name', settings.CODE)
            }
            ...
            for (i = 0; i < inputs.length; i++) {
                ...
                if (settings.CODE === 'DATE') {
                    inputs[i].setAttribute('class', 'datepicker_order form-control bx-soa-customer-input bx-ios-fix');
                    $(inputs[i]).datepicker({
                        minDate: new Date(),
                    }).data('datepicker').selectDate(new Date());
                }
                if (settings.CODE === 'TIME') {
                    inputs[i].setAttribute('style', 'display:none');
                    inputs[i].setAttribute('data-name', settings.CODE);
                    let select = BX.create('SELECT', {
                        props: {className: 'select_period'},
                    });
                    propContainer.appendChild(select);
                    $(select).select2();
                    for (i = 0; i < this.deliveryOptions.PERIOD_DELIVERY.length; i++) {
                        option = BX.create('OPTION', {
                            props: {
                                value: this.deliveryOptions.PERIOD_DELIVERY[i],
                                text: this.deliveryOptions.PERIOD_DELIVERY[i]
                            }
                        })
            ...
            let input = propContainer.querySelectorAll('input[type=checkbox]');
            for (i = 0; i < input.length; i++) {
                if (settings.CODE === 'CALL_ME') {
                    // Звонок курьера
                    propContainer.setAttribute('style', 'width: auto;margin-left: 1.3rem;margin-bottom: 0.8rem;')
                    input[i].setAttribute('class', 'form-check-input check_checkbox_delivery');
                    input[i].setAttribute('data-name', settings.CODE);
        ...
        editPropsComment: function (newBlock) {
        ...
        createTotalUnit: function (name, value, params) {
            ...
            return BX.create('DIV', {
                ...
                children: [
                    BX.create('SPAN', {props: {className: 'bx-soa-cart-t'}, html: name}),
                    BX.create('SPAN', {
                        props: {
