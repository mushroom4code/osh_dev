...
<script>
	...
	function CheckCity( type_check )
	{
		...
		let inp = $('.box_code_LOCATION  .dropdown-field').val();
		
		console.log('CheckCity:'+title+'|'+inp+'+START');
		if( type_check == 1 && inp == 166)
		{
			console.log('CheckCity:'+title+'|'+inp+'+OK');
			return true;
		}
		else if(
		title == 'Зеленоград, Москва, Россия'
		|| title == 'Щербинка, Москва, Россия'
		|| title == 'Одинцово, Одинцовский район, Московская область, Россия'			
		)
		{
			console.log('CheckCity:'+title+'|'+inp+'+OK TITLE');
			return true;
		}
		return false;
	}
	function CheckDate( type_check )
	{
		...
		console.log('CheckDate '+type_check+'=='+$(".bx-ui-sls-fake").attr('title')+'|'+$(".bx-ui-sls-fake").val()+'|'+$('.box_code_LOCATION  .dropdown-field').val());
	...
	function CheckViewDelivery(type_check)
	{
		...
		let DISTANCE = $('#DISTANCE').val();
		let DAYS_DELIVERY = $('#DAYS_DELIVERY').val();
		let DATE_ORDER = $('#DATE_ORDER').data('js-date');
		...
		else if( CheckDay == 0 && $('#ORDER_PROP_7').val() != '' )
		{
	...
	$(document).ready(function() {
		...
		//Сохраняем выбранный вариант доставки
		$(document).on('change', 'input[name="DELIVERY_ID"]', function(){
			//let DELIVERY_ID = $(this).find('input[name="DELIVERY_CHECK"]').val()
			/*console.log('CHECK DELIVERY: '+$(this).val());
			if( $(this).val() != 1 )
		...
		$(document).on('click', '.date_block', function()
		{
			...
			$('.DATE_ORDER').val(date);
			$('.DATE_ORDER').data('js-date', js_date);
