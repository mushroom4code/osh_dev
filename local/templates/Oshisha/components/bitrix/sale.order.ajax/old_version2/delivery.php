...
<div class="makeorder_content_left_delivery bx_section_delivery" <?if( $DELVERY_PAY == 1):?>style="display:none;"<?endif;?>>
	<?
	if(!empty($arResult["DELIVERY"]))
	{
		$width = ($arParams["SHOW_STORES_IMAGES"] == "Y") ? 850 : 700;
		?>
		<h4>2. Способ получения</h4>
		<?global $gorod;?>
		<div class="wrap_citiy">
			<?=$gorod?>
		</div>
		<?
		...
		foreach ($arResult["DELIVERY"] as $delivery_id => $arDelivery)
		{
			...
			if( $DELVERY_PAY == 1 && $arDelivery['ID'] != 2 ) continue;
			if( $DELVERY_PAY == 1 && $arDelivery['ID'] == 2 ) $arDelivery["CHECKED"] = 'Y';
			if( $arDelivery["PRICE"] == 99999999 && $arDelivery["CHECKED"] == "Y"){
				$NEXT_CHECKED = 1;
				
			}  
			if( $arDelivery["PRICE"] == 99999999)
				continue;
