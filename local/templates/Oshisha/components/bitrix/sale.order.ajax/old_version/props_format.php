...
//print_r($arProperties);
				foreach ($arSource as $arProperties)
				{
					...
					if( count($PROP_iskl) > 0 && in_array($arProperties["ID"], $PROP_iskl))
						...
						if( in_array($arProperties["CODE"], array('ADDRESS', 'CAR_MODEL', 'CAR_NUMBER', 'FIO_POLUCHATELYA', 'TIME', 'CALL_ME', 'DATE', 'FIO_PROPUSK')) )
							ob_start();	
						...
						if( in_array($arProperties["CODE"], array('EMAIL', 'PHONE')) )
						{
							...
							if( stripos($arProperties["VALUE"],'noemail.sms') !== false)
							{
								$arProperties["VALUE"] = '';
							}
						}
						$k++;					
						if( $_POST["is_ajax_post"] == 'Y' && ( !$_POST['DISTANCE'] && !$_POST['TYPE_DISTANCE']) && $arProperties["CODE"] == 'DATE' )
							$style="display:none;"
					?>
					<div data-property-id-row="<?=intval(intval($arProperties["ID"]))?>" style="<?=$style?>" class="box_code_<?=$arProperties["CODE"]?> form-group box<?=intval(intval($arProperties["ID"]))?> bx-soa-customer-field p-0 <?=$dop_class?>">
						...
						elseif ($arProperties["TYPE"] == "TEXT")
						{
							...
							<?
								...
								$i = 0;
							?>
							<div class="date_block_wrap_wait" style="display:none;">Секундочку, подбираем ближайшие даты доставки</div>
							<div class="date_block_wrap">
								<?for($k=0;$k<14;$k++):?>
									...
									<div class="date_block <?=$SELECTED?>" data-day="<?=date('N', strtotime($dateStartDelivery) + ($k * 24 * 3600));?>"  data-js-date="<?=date('Y-m-d', strtotime($dateStartDelivery) + ($k * 24 * 3600));?>" data-date="<?=date('d.m.Y', strtotime($dateStartDelivery) + ($k * 24 * 3600));?>">
										<?=FormatDate('d.m, D', strtotime($dateStartDelivery) + ($k * 24 * 3600));?>
									</div>
									<?if( $i == 5) break;?>
								<?endfor;?>
								<? if( $_POST["is_ajax_post"] != 'Y') $arProperties["VALUE"] = ''; 
								global $SELECTED_DAY; $SELECTED_DAY = $arProperties["VALUE"]; //Порбрасывается в delivery.php
								?>
								<input type=hidden class="DATE_ORDER" data-js-date="<?if($arProperties["VALUE"] != ''):?><?=date("Y-m-d",strtotime($arProperties["VALUE"]))?><?endif;?>" id="DATE_ORDER" name="<?=$arProperties["FIELD_NAME"]?>" value="<?=$arProperties["VALUE"]?>">
								</div>
							<?else:?>							
							<?
							if( $arProperties["CODE"] == 'ADDRESS' && $_POST["is_ajax_post"] != 'Y')
								$arProperties["VALUE"] = '';
							?>
							<?if( stripos($arProperties["VALUE"],'noemail.sms') !== false) $arProperties["VALUE"] = '';?>
							
							<div class="soa-property-container">
								<input type="text" data-name="<?=$arProperties["CODE"]?>" autocomplete="off" size="<?=$arProperties["SIZE1"]?>" value="<?=$arProperties["VALUE"]?>" class="form-control bx-soa-customer-input bx-ios-fix block_code_<?=$arProperties["CODE"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" />
								<?if (strlen(trim($arProperties["DESCRIPTION"])) > 0):?>
									<div class="bx_description"><?=$arProperties["DESCRIPTION"]?></div>
								<?endif?>
							</div>
						...
						elseif ($arProperties["TYPE"] == "LOCATION")
						{
							...
							<div class="bx_block r3x1">
								...
								<?endif?>
	<input type="text" name="<?=$arProperties["FIELD_NAME"]?>" value="<?=$value?>">
					...
					<?
						...
						if( in_array($arProperties["CODE"], array('TIME', 'CALL_ME', 'DATE')) )
