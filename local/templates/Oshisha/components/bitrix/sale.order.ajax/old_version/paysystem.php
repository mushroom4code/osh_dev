<? if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die(); ?>
	...
	<?foreach ($arResult["PAY_SYSTEM"] as $arPaySystem):
		...
		?>
		<input type="hidden" name="PAY_SYSTEM_ID" class="PAY_SYSTEM_ID" value="<?=$arPaySystem["ID"]?>">
		<?}
	endforeach;?>	
	<div id="bx-soa-paysystem" class="bx-soa-section mb-4 bx-active bx-selected">
		...
		<?
			...
			foreach ($arResult["PAY_SYSTEM"] as $arPaySystem) {
				...
				?>
						<div data-pay-id="<?=$arPaySystem["ID"]?>" class="bx-soa-pp-company bx-soa-pp-company-paysistem col-6 <?if( $arPaySystem["CHECKED"] == 'Y'):?>bx-selected<?endif;?>">
							<div class="bx-soa-pp-company-graf-container pay_system height_100">
								<div  class="bx-soa-pp-company-title" for="ID_PAY_SYSTEM_ID_<?= $arPaySystem["ID"] ?>">
									<?
				...
				if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) == 0 && intval($arPaySystem["PRICE"]) == 0) {
					if (count($arResult["PAY_SYSTEM"]) == 1) {
					?>
						<div class="bx-soa-pp-company bx-soa-pp-company-paysistem col-6 <?if( $arPaySystem["CHECKED"] == 'Y'):?>bx-selected<?endif;?>">
							<div class="bx-soa-pp-company-graf-container pay_system height_100">
