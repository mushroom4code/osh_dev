;(function () {
    ...
    BX.Sale.BasketComponent = {
        ...
        init: function (parameters) {
            ...
            $('.select_sort_basket').on('change', this.sortProduct.bind(this))
        },
            ...
            BX.ajax({
                ...
                onsuccess: BX.delegate(function (result) {
                    ...
                    if (result.DELETED_BASKET_ITEMS) {
                        //TODO recalculate count and fix delete baskets
        ...
        createBasketItem: function (itemId) {
            ...
            basketItemTemplate = this.listTemplate === 'line'
                ...
                : this.getTemplate('basket-item-grid-template');

            if (basketItemTemplate) {
                basketItemHtml = this.renderBasketItem(basketItemTemplate, this.items[itemId]);
        ...
        redrawBasketItemNode: function (itemId) {
            ...
            var basketItemTemplate = this.listTemplate === 'line'
                ? this.getTemplate('basket-item-template')
                : this.getTemplate('basket-item-grid-template');
            ...
            if (basketItemTemplate) {
                ...
                let basketItems = parentBox.querySelectorAll('div.basket-items-list-item-descriptions');
                if (basketItems.length === 0) {
                    BX.remove(parentBox);
                }

                basketItems = BX(this.ids.basketRoot).querySelectorAll('div.basket-items-list-item-descriptions');
                if (basketItems.length === 0) {
                    BX.remove(BX(this.ids.basketRoot));
                    BX.toggleClass(BX(this.ids.emptyBasket), ['d-none']);
                }
        ...
        sortProduct: function () {
            let val = document.querySelector('.select_sort_basket').value, i;
            this.arraySortProduct = [];
            for (i in this.items) {
                this.arraySortProduct.push(this.items[i]);
            }
            this.arraySortProduct.sort(function (a, b) {
                if (val === 'price_min') {
                    if (a.SUM_PRICE < b.SUM_PRICE) {
                        if (b.SORT === 0) {
                            a.SORT = 0
                        } else {
                            a.SORT = parseInt(b.SORT) - 100;
                        }
                        return -1;
                    }
            ...
            $('.basket-items-list-table').empty();
            for (i = 0; i < this.arraySortProduct.length; i++) {
                this.createBasketItem(this.arraySortProduct[i].ID);
            }
        },
        ...
        bindQuantityEvents: function (node, data) {
            ...
            let block = this.getEntity(node, 'basket-item-quantity-block');
            if (block) {
        ...
        renderBasketItem: function (template, data) {
            ...
            if (BX.type.isPlainObject(clonedData)) {
                let array_key = this.arraySortProduct;
                if (array_key.length !== 0) {
                    let i;
                    this.sortedItems = [];
                    for (i = 0; i < array_key.length; i++) {
                        this.sortedItems.push(array_key[i].ID);
                    }
                }
            }
