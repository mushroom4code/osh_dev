;(function(){
	...
	BX.Sale.BasketFilter.prototype.highlightSearchMatch = function(itemData)
	{
		...
		if (searchMatch)
		{
			...
			var entity, i, k, code;
			let item = document.querySelector('#basket-item-'+itemData.ID);
			let parent_item = BX.findParent(item, {tagName: 'div', className: 'category'});
			console.log(parent_item);
			parent_item.setAttribute('style','display:block');
			let parent_old =  BX.findParent(parent_item, {tagName: 'div', className: 'box'});
			let id_parent = parent_old.getAttribute('id');
			let parent = document.querySelectorAll('.box');
			for(i= 0; i < parent.length; i++){
				let id_cat = parent[i].getAttribute('id');
				if(id_cat !== id_parent){
					parent[i].setAttribute('style','display:none');
				}else {
					parent[i].setAttribute('style','display:block');
				}
			}
