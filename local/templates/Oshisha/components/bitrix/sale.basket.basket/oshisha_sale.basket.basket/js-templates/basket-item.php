...
<script id="basket-item-template" type="text/html">
    ...
    <div class="basket-items-list-item-descriptions w-full flex flex-row flex-wrap justify-between">
        <div class="basket-items-list-item-descriptions-inner flex flex-row p-0  md:w-9/12
         {{#NOT_AVAILABLE}} w-10/12 {{/NOT_AVAILABLE}} {{^NOT_AVAILABLE}} w-9/12 {{/NOT_AVAILABLE}}"
             id="basket-item-height-aligner-{{ID}}">
            <?php if (in_array('PREVIEW_PICTURE', $arParams['COLUMNS_LIST'])) { ?>
                <div class="basket-item-block-image md:h-auto md:p-3 p-2 h-fit bg-white min-w-max rounded-lg flex justify-center align-center border md:mr-4 mr-2">
                    {{#DETAIL_PAGE_URL}}
                    <a href="{{DETAIL_PAGE_URL}}" class="basket-item-image-link h-fit">
                        {{/DETAIL_PAGE_URL}}
                        <img class="basket-item-image md:h-32 md:w-32 w-12 h-12 object-contain" alt="{{NAME}}"
                        ...
                        <div class="basket-item-label-text basket-item-label-big <?= $labelPositionClass ?>">
                            {{#LABEL_VALUES}}
                            <div
                                    {{#HIDE_MOBILE}} class="md:hidden block" {{
                            /HIDE_MOBILE}}>
            ...
            {{/SHOW_LOADING}}
            <div class="basket-item-block-info flex flex-col justify-between md:w-inherit w-auto">
                <div>
                    ...
                    <h2 class="basket-item-info-name">
                        ...
                        <a href="{{DETAIL_PAGE_URL}}" class="basket-item-info-name-link">
                            ...
                            <span data-entity="basket-item-name"
                                  class="font-semibold dark:font-light md:text-lg text-textLight text-xs
                                  dark:text-textDarkLightGray md:line-clamp-none line-clamp-1">{{NAME}}</span>
                            {{#DETAIL_PAGE_URL}}
                        </a>
                        {{/DETAIL_PAGE_URL}}
                    </h2>
                                ...
                                switch (trim((string)$blockName)) {
                                    case 'props':
                                        if (in_array('PROPS', $arParams['COLUMNS_LIST'])) { ?>
                                            {{#PROPS}}
                                            {{#VKUS.length}}
                                            <div class="relative toggle_taste h-inherit md:w-auto w-40 md:block hidden">
                                                <div class="variation_taste flex flex-wrap flex-row md:my-2 my-1 js__tastes-list md:h-7 h-5 overflow-auto">
                                                    {{#PROPS}}
                                                    {{#VKUS}}
                ...
                <div class="flex flex-row items-center relative">
                    {{^NOT_AVAILABLE}}
                    <div class="basket-items-list-item-amount justify-between items-end md:mr-4 mr-2">
                        <div class="flex flex-row">
                            <div class="basket-item-block-amount{{#NOT_AVAILABLE}} disabled {{/NOT_AVAILABLE}}
                                flex flex-row items-center justify-between w-full"
                                 data-entity="basket-item-quantity-block">
                                {{^GIFT}}
                                <span class="basket-item-amount-btn-minus rounded-full md:py-0 md:px-0 py-3 px-1.5
                                ...
                                <div class="basket-item-amount-filed-block">
                                    <input type="text" class="product-item-amount dark:bg-tagFilterGray bg-textDarkLightGray
                                        focus:border-none text-center border-none md:text-sm text-xs
                                        shadow-none md:py-2.5 py-2 px-3 md:mx-2 mx-1 outline-none rounded-md md:w-14 w-12"
                                           value="{{QUANTITY}}"
                                ...
                                <span class="basket-item-amount-btn-plus no-select cursor-pointer flex items-center
                                            justify-center rounded-full md:p-0 p-1 dark:bg-dark md:dark:bg-darkBox
                    ...
                    <div class="bx_catalog_item_controls">
                        <svg class="detail_popup md:w-[34px] md:h-[33px] w-6 h-[23px]
                             <?= $is_key_found ? 'subscribed stroke-light-red' : ' stroke-black dark:stroke-white' ?>"
                    ...
                    <?php if ($useActionColumn) { ?>
                        <div class="flex flex-row like-column ml-2 items-center">
                            ...
                            <div class="basket-items-list-item-remove md:ml-4 ml-2">
                                <div class="basket-item-block-actions">
                                    <span class="basket-item-actions-remove" title="Удалить"
                                          data-entity="basket-item-delete">
                                        <svg class="md:h-7 h-6 md:w-6 w-5" viewBox="0 0 28 32" fill="none"
                                             xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="flex flex-col justify-between items-end end-column md:w-3/12
        {{#NOT_AVAILABLE}} w-2/12 {{/NOT_AVAILABLE}} {{^NOT_AVAILABLE}} w-3/12 {{/NOT_AVAILABLE}}">
                <div class="flex flex-row justify-between">
                    <div class="flex flex-col price-column ml-2">
                        <div class="basket-items-list-item-price md:mb-3 mb-1 <?= (!isset($mobileColumns['SUM']) ? 'md:hidden block' : '') ?>">
