...
foreach ($arParams['SECTIONS_ITEMS'] as $sectionId => $section) { ?>
    <?php if ($APPLICATION->GetCurPage() === '/hit/') { ?>
    ...
    <div class="by-card mb-3">
        ...
        <div id="<?= $strContID; ?>"
            ...
            <div class="bx_catalog_tile_section" data-init="<?= count($sectionBrands) ?>">
                <?php
                $boolFirst = true;
                $arRowIDs = array();
        ...
        <?php if (count($section) > 4 && $APPLICATION->GetCurPage() === '/hit/') { ?>
            <div class="text-center mt-4" data-entity="lazy-container-1">
                <a href="/hit/<?= $arParams['SECTIONS'][$sectionId]['CODE'] ?>/"
                   class="btn text_catalog_button link_red_button btn-md" style="margin: 15px;" data-use="show-more-1">
                    Смотреть все </a>
            </div>
        <?php } ?>
    </div>
