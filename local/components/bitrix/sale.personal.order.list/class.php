...
class CBitrixPersonalOrderListComponent extends CBitrixComponent
{
	...
	protected function processRequest()
	{
        ...
        //enterego added empty basket from request
        $this->requestData["EMPTY_BASKET"] = ($_REQUEST["EMPTY_BASKET"] == "Y");
        //enterego end
	...
	private function getRealId($id)
	{
        ...
        //enterego deleted LID from filter
		$filter = array(
			'select' => array("ID"),
	...
	protected function copyOrder2CustomerBasket($id)
	{
		...
		if ($id)
		{
            //enterego check if in need to empty the basket
            if ($this->requestData["EMPTY_BASKET"]) {
            ...
            //enterego deleted option to select only orderable items
			...
			foreach ($oldBasketItems as $oldBasketItem)
			{
				...
				else
				{
                    ...
                    //enterego setting the subscribe option to be to push unavailable items to basket
                    $item->setField('SUBSCRIBE', 'Y');
                    //enterego end
