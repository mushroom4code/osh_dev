...
<div class="yandex-share-panel flex flex-col mt-3 <?=$arParams["TEXT_ALIGN"]?>">
	<?if(trim($arParams["TEXT_BEFORE"])):?><div class="ya-share-text-wrap mb-1"><?=$arParams["TEXT_BEFORE"]?></div><?endif;?>
	<div class="ya-share-wrap mb-1 flex flex-col">
        <div class="ya-share2 flex flex-col" data-services="<?=implode(',', $arParams["SERVISE_LIST"])?>"
            <?=$arParamsShow[$arParams["VISUAL_STYLE"]]?> <?=$dataline?>>
        </div>
    </div>
</div>
