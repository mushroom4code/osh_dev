...
class CIBlockCMLImport
{
	...
	function ImportProperties($XML_PROPERTIES_PARENT, $IBLOCK_ID)
	{
		...
		while($ar = $rs->Fetch())
		{
			...
			//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/_testHL.txt', print_r($arProperty,1).$isExternal.PHP_EOL, FILE_APPEND);
			if(array_key_exists($arProperty["XML_ID"], $arElementFields))
				continue;
	...
	function ImportPropertyDirectory($arProperty, $arEnumXmlNodes)
	{
		if (!Loader::includeModule('highloadblock'))
			return true;
		//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/_testHL.txt', print_r($arProperty,1).PHP_EOL, FILE_APPEND);
		...
		$rsData = $entity_data_class::getList(array(
			"select" => array("ID", "UF_NAME", "UF_XML_ID", "UF_SORT", "UF_DESCRIPTION", "UF_FILE", "UF_DESCR"),
		));
		...
		while($obR = $resU->Fetch())
		{
			$arEnumMapIB[$obR["XML_ID"]] = $obR['ID'];
		}	
		$i = 0;
		foreach($arEnumXmlNodes as $arE)
		{
			...
			{
				$arE = $this->_xml_file->GetAllChildrenArray($arE);
				//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/_testHL2.txt', print_r($arE,1).PHP_EOL, FILE_APPEND);   
				if(
				...
				)
				{
					...
					$arFields = array(
						...
						"UF_DESCRIPTION" =>  $arE[$this->mess["IBLOCK_XML2_DESCRIPTION"]],
						"UF_DESCR" =>  $arE[$this->mess["IBLOCK_XML2_DESCRIPTION"]],
					);
					if (isset($arE[$this->mess["IBLOCK_XML2_PICTURE"]]))
					{
						$arFields["UF_FILE"] = $this->MakeFileArray($arE[$this->mess["IBLOCK_XML2_PICTURE"]]);
					}
					if( !isset($arFields["UF_FILE"]) )
					{
						$arFields["UF_FILE"] = CFile::MakeFileArray($_SERVER['DOCUMENT_ROOT'].'/upload/1c_catalog/'.$arE[$this->mess["IBLOCK_XML2_PICTURE"]]);
					}
					/*if( file_exists($_SERVER['DOCUMENT_ROOT'].'/upload/1c_catalog/'.$arE[$this->mess["IBLOCK_XML2_PICTURE"]]))
					{
						file_put_contents($_SERVER['DOCUMENT_ROOT'].'/_testHL235.txt', $_SERVER['DOCUMENT_ROOT'].'/upload/1c_catalog/'.$arE[$this->mess["IBLOCK_XML2_PICTURE"]].PHP_EOL, FILE_APPEND);    

					}*/
//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/_testHL234.txt', print_r($arFields["UF_FILE"],1).'|'.$arE[$this->mess["IBLOCK_XML2_PICTURE"]].PHP_EOL, FILE_APPEND); 
					$el = new CIBlockElement;
					//$arFieldsIblock = $arFields;
					$arFieldsIblock = [];
					$arFieldsIblock['IBLOCK_ID'] = 25; 
					$arFieldsIblock['ACTIVE'] = 'Y';
					$arFieldsIblock['XML_ID'] = $arFields['UF_XML_ID'];
					$arFieldsIblock['NAME'] = $arFields['UF_NAME'];
					$arFieldsIblock['PREVIEW_TEXT'] = $arFields['UF_DESCRIPTION'];
					$arFieldsIblock['PREVIEW_PICTURE'] = $arFields['UF_FILE'];
					
					if( !isset($arEnumMapIB[$arFieldsIblock['XML_ID']]) )
					{
						$RES_ID = $el->Add($arFieldsIblock);
						$arEnumMapIB[$arFieldsIblock['XML_ID']] = $RES_ID; 
					}
					else
						$el->Update($arEnumMapIB[$arFieldsIblock['XML_ID']], $arFieldsIblock);

					
					file_put_contents($_SERVER['DOCUMENT_ROOT'].'/_testHLIB2_enum.txt', print_r($arEnumMapIB,1).PHP_EOL, FILE_APPEND);   
					if(!array_key_exists($xml_id, $arEnumMap))
					{
						//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/_testHLIB2_Add.txt', print_r($arFields,1).PHP_EOL, FILE_APPEND);  
						$entity_data_class::add($arFields);
					}
					elseif( !empty($arFields['UF_FILE']) )//if ($arEnumMap[$xml_id]["UF_NAME"] !== $arFields['UF_NAME'])
					{
						//file_put_contents($_SERVER['DOCUMENT_ROOT'].'/_testHLIB2_Save.txt', $arEnumMap[$xml_id]["ID"].print_r($arFields,1).PHP_EOL, FILE_APPEND);  
	...
	function ImportElement($arXMLElement, &$counter, $bWF, $arParent)
	{
		...
		elseif(isset($arXMLElement[$this->mess["IBLOCK_XML2_NAME"]]))
		{
			...
			if($arDBElement)
			{
				...
				if(intval($arElement["MODIFIED_BY"]) <= 0 && $this->currentUserId > 0)
	...
	function ImportSection($xml_tree_id, $IBLOCK_ID, $parent_section_id)
	{
		...
		if($arDBSection = $rsSection->Fetch())
		{
			...
			if($bChanged)
			{
				...
				if(intval($arSection["MODIFIED_BY"]) <= 0 && $this->currentUserId > 0)
