...
//TODO enterego - redirect to main language file (bitrix/modules/iblock/lang/ru/classes/general/cml2_ee.php)
IncludeModuleLangFile(__FILE__);

class CIBlockCMLImportEe
{
	...
	function ImportElement($arXMLElement, &$counter, $bWF, $arParent)
	{
		...
		elseif(isset($arXMLElement[$this->mess["IBLOCK_XML2_NAME"]]))
		{
			...
			if(array_key_exists($this->mess["IBLOCK_XML2_PICTURE"], $arXMLElement))
			{
                ...
                // TODO Enterego Remove Picture

                \CModule::IncludeModule("iblock");
                $arFilter = Array("EXTERNAL_ID" => $arElement["XML_ID"]);
                $res = CIBlockElement::GetList(array(), $arFilter,false, array("nPageSize"=>50), array("ID"));

                while($ob = $res->GetNextElement())
                {
                    if($arData = $ob->GetFields()){
                        $ELEMENT_ID = $arData['ID'];
                        $IBLOCK_ID = $arData['IBLOCK_ID'];
                        $el = new \CIBlockElement;
                        $el->Update($ELEMENT_ID, array("PR EVIEW_PICTURE" => array('del' => 'Y'), "DETAIL_PICTURE" => array('del' => 'Y')), false, false);
