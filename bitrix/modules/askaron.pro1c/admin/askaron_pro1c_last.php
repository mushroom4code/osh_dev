...
else
{
	...
	{	
		...
		)
		{
			...
			$index = trim($_REQUEST["askaron_pro1c_index"]);
			$date = trim($_REQUEST[ "askaron_pro1c_date_".$index ]); // different index for javascript calendar bug
		...
		<tr>
			...
			<td width="50%" style="vertical-align: top">
				<form name="" action="" method="POST">
					...
					<?echo CAdminCalendar::CalendarDate(  "askaron_pro1c_date_".$index , CAskaronPro1C::GetDateOptionFromTimestamp( "sale", "last_export_time_committed_".$sub_path, ""), 10, true );?>&nbsp;&nbsp;<input type="submit" name="askaron_pro1c_action" value="<?=GetMessage("askaron_pro1c_last_exchange_order_button");?>">
					<input type="hidden" name="askaron_pro1c_index" value="<?=$index?>" />
