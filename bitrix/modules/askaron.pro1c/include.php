...
class CAskaronPro1C
{
	...
	function OnBeforeIBlockPropertyUpdate(&$arFields)
	{
		if (self::IsExchange())
		{
			if ( COption::GetOptionString( "askaron.pro1c", "skip_update_property_name") == "Y" )
			{
				$IBLOCK_ID = $arFields["IBLOCK_ID"];
				if (!$IBLOCK_ID)
				{
					$res = \CIBlockProperty::GetByID($arFields["ID"]);
					if ($ar_res = $res->Fetch())
					{
				...
				if ($IBLOCK_ID > 0)
				{
					...
					if (
						$arCatalogInfo["ADDITIONAL"]["PRODUCT_IBLOCK_ID"] == $IBLOCK_ID
						||
						$arCatalogInfo["ADDITIONAL"]["OFFERS_IBLOCK_ID"] == $IBLOCK_ID
					)
					{
						if (isset($arFields["NAME"]))
						{
							self::log($arFields, 'OnBeforeIBlockPropertyUpdate - do not update property name');
							unset($arFields["NAME"]);
						}
					}
				}
...
//					}					
	...
	{
		if ( !\Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
		{
			if (self::IsExchange())
			{
	...
	// PRICE
	// new
	public static function handlerPriceOnAdd( \Bitrix\Main\Event $event )
	{
		if ( \Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
		{
			if (self::IsExchange())
			{
				...
				self::log($arParams, 'Bitrix\Catalog\Model\Price::handlerPriceOnAdd - $event->getParameters()');
			}
		}
	}

	// new
	public static function handlerPriceOnUpdate( \Bitrix\Main\Event $event )
	{
		if ( \Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
		{
			if (self::IsExchange())
			{
				$arParams = $event->getParameters();
				self::log($arParams, 'Bitrix\Catalog\Model\Price::handlerPriceOnUpdate - $event->getParameters()');
			}
		}
	}

	// new
	public static function handlerPriceOnDelete( \Bitrix\Main\Event $event )
	{
		if ( \Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
		{
			if (self::IsExchange())
			{
				$arParams = $event->getParameters();
				self::log($arParams, 'Bitrix\Catalog\Model\Price::handlerPriceOnDelete - $event->getParameters()');
			}
		}
	}
	...
	public static function handlerProductOnBeforeAdd( \Bitrix\Main\Event $event )
	{
//		if ( \Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
//		{
//			if (self::IsExchange())
//			{
//				self::log("handlerProductOnBeforeAdd");
//				$result = self::EmptyProduct($event);
//				return $result;
//			}
//		}
	}
	...
	public static function handlerProductOnBeforeUpdate( \Bitrix\Main\Event $event )
	{
		if ( \Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
		{
			if (self::IsExchange())
			{
				self::log("handlerProductOnBeforeUpdate");
				$result = self::EmptyProduct($event);
				return $result;
			}
		}
	...
	// new LOG
	//public static function handlerLogProductOnBeforeAdd( \Bitrix\Catalog\Model\Event $event)
	public static function handlerLogProductOnBeforeAdd( \Bitrix\Main\Event $event )
	{
		if ( \Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
		{
			if (self::IsExchange())
			{
				$arParams = $event->getParameters();
				self::log($arParams, 'Bitrix\Catalog\Model\Product::OnBeforeAdd - $event->getParameters()');
			}
		}
	}

	// new LOG
	//public static function handlerLogProductOnBeforeUpdate( \Bitrix\Catalog\Model\Event $event)
	public static function handlerLogProductOnBeforeUpdate(\Bitrix\Main\Event $event)
	{
		if ( \Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
		{
			if (self::IsExchange())
			{
				$arParams = $event->getParameters();
				self::log($arParams, 'Bitrix\Catalog\Model\Product::OnBeforeUpdate - $event->getParameters()');
			}
		}
	}

	// NEW from 17.6.0
	//private static function EmptyProduct( \Bitrix\Catalog\Model\Event $event )
	private static function EmptyProduct( \Bitrix\Main\Event $event )
	{
		$result = new \Bitrix\Main\Entity\EventResult;

//		$arParams = $event->getParameters();
//		self::log($arParams, '$result');
					...
					if ($ELEMENT_ID > 0 && \CModule::IncludeModule("iblock"))
					{
						...
						if ($arElementFileds = $res->Fetch())
						{
							...
							if (isset($arCatalogInfo["ADDITIONAL"]["LIST"][$arElementFileds["IBLOCK_ID"]]))
							{
								...
								$result->modifyFields( array('fields' => $arFields) );
							}
						}
					}
	...
	public static function OnBeforeProductAddHandler(&$arFields)
	{
		if ( !\Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
		{
			if (self::IsExchange())
			{
				self::EmptyProductOld(0, $arFields);
			}
		}
	}
	...
	public static function OnBeforeProductUpdateHandler($ID, &$arFields)
	{
		if ( !\Askaron\Pro1c\Tools::IsCatalog_17_6_0() )
		{
			if (self::IsExchange())
			{
				self::EmptyProductOld($ID, $arFields);
			}
		}
	}

	// old before 17.6.0
	private static function EmptyProductOld($ID, &$arFields)
	{
		if ( COption::GetOptionString( "askaron.pro1c", "quantity_set_to_zero") == "Y" )
		{
			// skip import.xml file
	...
	public static function OnEndBufferContentHandler( &$content )
	{
		if (self::IsExchange())
		{
			...
			if ( isset( $_GET["GZ_COMPRESSION_SUPPORTED"] ) && $_GET["GZ_COMPRESSION_SUPPORTED"] && function_exists("gzuncompress") )
			{
				$text = gzuncompress($content_converted);
				if ($text !== false)
				{
					$content_converted = "gzuncompress:\n".$text;
				}
			}
...
//				"QWERT" => " \" \' fskdjfslkj \n
//				 dsfsdjfsdj gkjdfkgjdf dgdkfj dfg",
			);

			\Askaron\Pro1c\Tools::SendLog( $arSendLogParams );
...
//			}
	...
	public static function log($value, $label = "")
	{
