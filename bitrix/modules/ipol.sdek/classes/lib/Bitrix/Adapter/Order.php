...
class Order
{
    ...
    public function uploadedOrder($bitrixId,$mode = false,$oldApp=false)
    {
        ...
        if($orderParams){
            ...
            switch (true){
                case ($orderParams["PVZ"]) : $addressTo->setField('pointId',$orderParams["PVZ"]); break;
                case ($orderParams["PST"]) : $addressTo->setField('pointId',$orderParams["PST"]); break;
                default : $addressTo->setCode($orderParams["location"])
            ...
            if(array_key_exists('sender_street',$orderParams) && $orderParams['sender_street']) {
                $sender->setFields(array(
                    'street' => (array_key_exists('sender_street',$orderParams) && $orderParams['sender_street']) ? $orderParams['sender_street'] : '',
