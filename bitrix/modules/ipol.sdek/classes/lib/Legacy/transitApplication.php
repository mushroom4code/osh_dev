...
";
		...
		if($order->getSender()->getField('realSeller'))
            $strXML .= "
		SellerName=\"".$sender->getField('realSeller')."\">
		";
        else
            $strXML .= ">
		";
        if($sender->getPhone()){
            ...
            $strXML .= ">
            ";
            if($sender->getField('street')) {
                $strXML .= "<Address Street=\"".$sender->getField('street')."\"";
                if($sender->getField('house')){
                    $strXML .= " House=\"".$sender->getField('house')."\"";
                }
                if($sender->getField('flat')){
                    $strXML .= " Flat=\"".$sender->getField('flat')."\"";
                }
                $strXML .= "/>
            ";
            }
    ...
    )
    {
        ...
        if (isset($this->requestType) && $this->requestType == PointsHandler::REQUEST_TYPE_BACKUP) {
            ...
            $request = \sdekHelper::sendToSDEK(false, 'pvzlist', $get);
        }
