    ...
    public function getButtons()
    {
        if (is_null($this->buttons)) {
            ...
            foreach ($this->getDefaultButtons() as $index => $button) {
                $this->buttons[$index] = array_merge($button, ['CAPTION' => Loc::getMessage($button['CAPTION'])]);
            }
        }
    ...
    public function getColumns()
    {
        if (is_null($this->columns)) {
            foreach ($this->getDefaultColumns() as $index => $column) {
                $this->columns[$index] = array_merge($column, ['name' => Loc::getMessage($column['name'])]);
            }
        }
    ...
    protected function getRowActions($item)
    {
        ...
        foreach ($actions as $index => $action) {
            $ret[$index] = array_merge($action, [
                'TEXT'    => $action['TEXT'],//Loc::getMessage($action['TEXT']),
