...
class BarcodeGeneratorCode128
{
     ...
     */
    public function __construct(string $code, int $density = 1)
    {
        self::getDimensions($code, $density);
        ...
        imagefill($this->image, 0, 0, self::getWhiteColor($this->image));
        imagesetthickness($this->image, $density);
        ...
        for ($i = 0, $x = 0, $inc = round(($density / 72) * 100); $i < strlen($enc_str); $i++) {
            ...
            $black = self::getBlackColor($this->image);
            for ($n = 0; $n < $val; $n++, $x += $inc) {
                if ($i % 2 == 0) {
                    imageline($this->image, $x, 0, $x, $this->height, $black);
                }
            }
     ...
     * @param $code
     */
    private function getDimensions($density, $code)
    {
        ...
        $this->width  = 200; // round($width * 72);
        $this->height = 30; // $height * 72;
    }
