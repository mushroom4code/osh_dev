		...
		if($strOfferField = $this->isOfferField($strField)){
			...
			if(!empty($arOffers)){
				...
				foreach($arOffers as $intOfferId){
					$this->processField($intOfferId, $strField, $this->get('value'), $this->get('description'),
						$intIBlockOffersId);
				}
				$bResult = true;
			}
			...
			$bResult = $this->processField($intElementId, $strField, $this->get('value'), $this->get('description'));
		}
		return $bResult;
	}
