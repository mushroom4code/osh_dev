			...
			if(typeof callback == 'function'){
				...
				if(div.length){
					...
					if(callbackResult !== true){
						...
						messageBody += "\n" + '- ' + '"' + title+ '"' + (typeof callbackResult == 'string' ? ': ' + "\n" + callbackResult : '');
						canStart = false;
					}
				}
			}
 ...
 */
/*
function wdaGetSelectedField(select){
	var value = false;
	select = $(select);
	if(select.length){
		if(select.val()){
...
wdaPopupLoadProfile.SetNavButtons = function(empty){
	...
	if(empty) {
		...
		container.append('<span><b>'+BX.message('WDA_POPUP_PROFILE_LOAD_NOTICE')+'</b></span>');
		container.append('<div style="clear:both"/>');
	}
}
		...
		if(!$('input[data-role="cron-tasks-cannot-autoset"]', this.PARTS.CONTENT_DATA).length){
			...
			'action': function(){
				var id = wdaGenerateString(),
					tasks = $(this.btn).closest('div.bx-core-window').find('div[data-role="cron-tasks"] table.adm-list-table tbody'),
...
$(document).delegate('div[data-role="entity-type"] input[type="radio"]', 'change', function(e, params){
	...
	if($(this).val() == 'section'){
		$('div[data-role="wda_filter_section_restrictions"]').show();
	}
	else{
		$('div[data-role="wda_filter_section_restrictions"]').hide();
	}
});
