...
class IBlock {
				...
				if($arIBlockType = $resIBlockType->getNext(false,false)){
					if($arIBlockType['SECTIONS']=='Y') {
						...
						foreach($arUserFields as $strFieldCode => $arUserField){
							...
							*/
							$strType = static::getSectionFieldType($arUserField['USER_TYPE_ID']);
	 ...
	 */
	public static function getElementArray($intElementId, $intIBlockId, $arFeatures=null, $bPropertiesById=true){
		if(!\Bitrix\Main\Loader::includeModule('iblock')){
			return [];
		}
		if(!$intIBlockId){
		...
		elseif(is_array($arFeatures) && !empty($arFeatures)){
			$bGetFields = is_array($arFeatures['FIELDS']) && !empty($arFeatures['FIELDS']);
		...
		if(is_object($obElement)){
			...
			# Get offers
			if($bGetOffers) {
				static::getElementOffers($arResult, $bGetFirstOffer);
			}
			# Get SEO for element
			if($bGetSeo){
				$arResult['SEO'] = [];
	 ...
	 */
	protected static function getElementOffers(&$arElement, $bGetFirstOffer){
		$arElement['OFFERS'] = [];
		...
		if(is_array($arCatalog) && $arCatalog['OFFERS_IBLOCK_ID'] && \Bitrix\Main\Loader::includeModule('iblock')) {
			...
			$arFilter['PROPERTY_'.$arCatalog['OFFERS_PROPERTY_ID']] = $arElement['ID'];
	 ...
	 *	Get all hl-enums
	 */
	public static function getHighloadItems($strHighloadTable, $arSort=null, $arFilter=null){
		$arResult = [];
		...
		if(strlen($strDataClass)){
			...
			while($arItem = $resSata->fetch()) {
				$arResult[$arItem['UF_XML_ID']] = $arItem;
			}
		}
		return $arResult;
	}
	...
	public static function getSDirectoryValuesByXmlId($strHighloadTable, $mXmlId){
		...
		if(is_array($mResult) && !empty($mResult) || strlen($mResult)){
			...
			$arItems = static::getHighloadItems($strHighloadTable, false, $arFilter);
			if(is_array($arItems)){
				if(is_array($mResult)){
	 ...
	 *	
	 */
	public static function getPropertyById($intPropertyId, $intIBlockId){
		if($intPropertyId){
			$resProp = \CIBlockProperty::getList([], ['IBLOCK_ID' => $intIBlockId, 'ID' => $intPropertyId]);
	...
	public static function getSectionList($arSort, $arFilter, $arSelect=null, $intLimit=null){
		if(\Bitrix\Main\Loader::includeModule('iblock')){
			if(static::isSectionD7Available()){
				# D7
				$mEntity = \Bitrix\IBlock\Model\Section::compileEntityByIBlock($arFilter['IBLOCK_ID']);
				if($mEntity){
