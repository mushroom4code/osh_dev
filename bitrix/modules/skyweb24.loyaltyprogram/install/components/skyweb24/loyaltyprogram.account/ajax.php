...
if($USER->IsAuthorized()){
	...
	\Bitrix\Main\Loader::IncludeModule("skyweb24.loyaltyprogram");
	if(isset($_REQUEST['writeoff_requisite']) && $_REQUEST['writeoff_bonus']>0){
		$cProfile=new Skyweb24\Loyaltyprogram\Components;
		if(!empty($_REQUEST['writeoff_requisite'])){
			$status=$cProfile->setWriteOffBonus($_REQUEST['writeoff_bonus'], $_REQUEST['writeoff_requisite']);
	...
	<?$APPLICATION->IncludeComponent(
		"skyweb24:loyaltyprogram.account",
		$tmpltName,
		$params
	);?>
	<?}
