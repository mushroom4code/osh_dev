	...
	for(let i=0; i<formSubmit.length; i++){
		...
		currentForm.addEventListener('submit', function(e){
			...
			let strSerialized = BX.ajax.prepareData(BX.ajax.prepareForm(this).data),
				self=this;;
			BX.ajax({
				url: '/bitrix/components/skyweb24/loyaltyprogram.room/ajax.php',
				data: strSerialized,
				method: 'POST',
				dataType: 'json',
				timeout:300,
	...
	updateTable:function(rows){
		if(this.table){
			...
			for(let nextRow of rows){
				let status=BX.message("skyweb24_loyaltyprogram_TABLE_SITE_STATUS_Y"),
					actionConfirm='',
					dateConfirm=nextRow['date_confirm'];
				if(nextRow['confirmed']=='N'){
					status=BX.message("skyweb24_loyaltyprogram_TABLE_SITE_STATUS_N");
					actionConfirm='<span class="fa fa-check" data-code="'+nextRow['code']+'" title="'+BX.message("skyweb24_loyaltyprogram_ACTION_CONFIRM")+'"></span>';
					dateConfirm='';
				}
				newTRs+='<tr data-id="'+nextRow.id+'"><td>'+nextRow.site+'</td><td>'+status+'</td><td>'+dateConfirm+'</td><td>'+actionConfirm+' <span class="fa fa-remove" title="'+BX.message("skyweb24_loyaltyprogram_ACTION_DELETE")+'"></span></td></tr>';
			}
			...
			for(let nextRow of remButtons){
				nextRow.addEventListener('click', function(){
					...
					siteManager.popupDelete = BX.PopupWindowManager.create("popup-delete", null, {
						content: BX.message('skyweb24_loyaltyprogram_TABLE_SITE_MESS_DELETE'),
						closeIcon: true,
						autoHide: true,
						 buttons:[
							new BX.PopupWindowButton({
								text: BX.message('skyweb24_loyaltyprogram_ACTION_DELETE'),
			...
			for(let nextRow of checkButtons){
				nextRow.addEventListener('click', function(){
					...
					siteManager.popupCheck = BX.PopupWindowManager.create("popup-delete", null, {
						content: BX.message('skyweb24_loyaltyprogram_TABLE_SITE_MESS_CHECK_1')+'<br><br><b>'+this.dataset.code+'.txt</b><br><br>'+BX.message('skyweb24_loyaltyprogram_TABLE_SITE_MESS_CHECK_2'),
						closeIcon: true,
						autoHide: true,
						 buttons:[
							new BX.PopupWindowButton({
								text: BX.message('skyweb24_loyaltyprogram_ACTION_CONFIRM'),
								className: "popup-window-button-blue",
								events: {click: function(){
					...
					siteManager.popupCheck.setContent(BX.message('skyweb24_loyaltyprogram_TABLE_SITE_MESS_CHECK_1')+'<br><br><b>'+this.dataset.code+'.txt</b><br><br>'+BX.message('skyweb24_loyaltyprogram_TABLE_SITE_MESS_CHECK_2'));
