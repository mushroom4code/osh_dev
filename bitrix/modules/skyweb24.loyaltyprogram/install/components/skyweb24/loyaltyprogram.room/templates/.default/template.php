...
CJSCore::RegisterExt('loyalty', ['lang'=>'/bitrix/components/skyweb24/loyaltyprogram.room/templates/.default/lang/'.LANGUAGE_ID.'/template.php']);
...
ob_start();?>
	<div class="lpBindingTools">
		...
		<?if(!empty($arResult['PARTNER_SITE'])){?>
			...
			<div class="siteItem">
			<a class="link add_site" href="javascript:void(0);"><?=GetMessage("skyweb24_loyaltyprogram_ADD_SITE")?></a><br>
			<form style="display:none;" name="add_site">
				<?=bitrix_sessid_post()?>
				<span class="info_error"></span>
				<input type="hidden" name="ajax" value="Y" />
				<input type="text" name="newSite" />
				<button type="submit"><?=GetMessage("skyweb24_loyaltyprogram_BUTTON_SITE_SUBMIT")?></button>
				<button type="button"><?=GetMessage("skyweb24_loyaltyprogram_BUTTON_CANCEL")?></button>
			</form>
			<? $tableVisible=count($arResult['PARTNER_SITE']['SITES'])==0?' style="display:none;"':''; ?>
			<table<?=$tableVisible?>>
				<thead>
					<tr>
						<td><?=GetMessage("skyweb24_loyaltyprogram_TABLE_SITE_SITE")?></td>
						<td><?=GetMessage("skyweb24_loyaltyprogram_TABLE_SITE_STATUS")?></td>
						<td><?=GetMessage("skyweb24_loyaltyprogram_TABLE_SITE_DATE_CONFIRMED")?></td>
						<td><?=GetMessage("skyweb24_loyaltyprogram_TABLE_SITE_ACTION")?></td>
					</tr>
				</thead>
				<tbody><?if(count($arResult['PARTNER_SITE']['SITES'])>0){foreach($arResult['PARTNER_SITE']['SITES'] as $nextSite){
					$status=GetMessage("skyweb24_loyaltyprogram_TABLE_SITE_STATUS_Y");
					$actionConfirm='';
					$dateConfirm=$nextSite['date_confirm'];
					if($nextSite['confirmed']=='N'){
						$status=GetMessage("skyweb24_loyaltyprogram_TABLE_SITE_STATUS_N");
						$actionConfirm='<span data-code="'.$nextSite['code'].'" class="fa fa-check" title="'.GetMessage("skyweb24_loyaltyprogram_ACTION_CONFIRM").'"></span>';
						$dateConfirm='';
					}
					?>
					<tr data-id="<?=$nextSite['id']?>">
						...
						<td><?=$status?></td>
						<td><?=$dateConfirm?></td>
						<td><?=$actionConfirm?> <span class="fa fa-remove" title="<?=GetMessage("skyweb24_loyaltyprogram_ACTION_DELETE")?>"></span></td>
					</tr>
				<?}}?></tbody>
			</table>
			</div>
		...
		<div>
			...
			<div class="refLink">
				...
				<span class="link" onclick="show_url(this);"><?=GetMessage("skyweb24_loyaltyprogram.CLICK_ON_THE_LINK")?></span>
				<span class="link sub_url" style="display:none;"><?=GetMessage("skyweb24_loyaltyprogram.REFERENCE_IS_COPIED")?></span>
		...
		<div class="socialButtons" id="socialButtons">
			...
			<a href="<?=$arResult['SOCIAL']['VK']?>" target="_blank" title="<?=GetMessage("skyweb24_loyaltyprogram_SHARE_IN_VK")?>"><img src="<?=$templateFolder?>/img/vk.jpg"></a>
			<?}?>
			<?if(isset($arResult['SOCIAL']['ODNOKLASSNIKI'])){?>
			<a href="<?=$arResult['SOCIAL']['ODNOKLASSNIKI']?>" target="_blank" title="<?=GetMessage("skyweb24_loyaltyprogram_SHARE_IN_OK")?>"><img src="<?=$templateFolder?>/img/odnoklassniki.jpg"></a>
			<?}?>
			<?if(isset($arResult['SOCIAL']['FACEBOOK'])){?>
			<a href="<?=$arResult['SOCIAL']['FACEBOOK']?>" target="_blank" title="<?=GetMessage("skyweb24_loyaltyprogram_SHARE_IN_FB")?>"><img src="<?=$templateFolder?>/img/faceb.jpg"></a>
			<?}?>
			<?if(isset($arResult['SOCIAL']['TWITTER'])){?>
			<a href="<?=$arResult['SOCIAL']['TWITTER']?>" target="_blank" title="<?=GetMessage("skyweb24_loyaltyprogram_SHARE_IN_TWITTER")?>"><img src="<?=$templateFolder?>/img/tw.jpg"></a>
			<?}?>
			<?if(isset($arResult['SOCIAL']['GOOGLE'])){?>
			<a href="<?=$arResult['SOCIAL']['GOOGLE']?>" target="_blank" title="<?=GetMessage("skyweb24_loyaltyprogram_SHARE_IN_GOOGLE")?>"><img src="<?=$templateFolder?>/img/google.jpg"></a>
			<?}?>
			<?if(isset($arResult['SOCIAL']['MOYMIR'])){?>
			<a href="<?=$arResult['SOCIAL']['MOYMIR']?>" target="_blank" title="<?=GetMessage("skyweb24_loyaltyprogram_SHARE_IN_MM")?>"><img src="<?=$templateFolder?>/img/mail.jpg"></a>
			<?}?>
			<?if(isset($arResult['SOCIAL']['WHATSAPP'])){?>
			<a href="<?=$arResult['SOCIAL']['WHATSAPP']?>" target="_blank" title="<?=GetMessage("skyweb24_loyaltyprogram_SHARE_IN_WHATSAPP")?>"><img src="<?=$templateFolder?>/img/whatsapp.jpg"></a>
			<?}?>
			<?if(isset($arResult['SOCIAL']['VIBER'])){?>
			<a href="<?=$arResult['SOCIAL']['VIBER']?>" target="_blank" title="<?=GetMessage("skyweb24_loyaltyprogram_SHARE_IN_VIBER")?>"><img src="<?=$templateFolder?>/img/viber.jpg"></a>
			<?}?>
			<?if(isset($arResult['SOCIAL']['TELEGRAM'])){?>
			<a href="<?=$arResult['SOCIAL']['TELEGRAM']?>" target="_blank" title="<?=GetMessage("skyweb24_loyaltyprogram_SHARE_IN_TELEGRAM")?>"><img src="<?=$templateFolder?>/img/telegram.jpg"></a>
			<?}?>
			<?/*if(isset($arResult['SOCIAL']['EMAIL'])){?>
			<a href="javascript:void(0);" onclick="loyaltyEmail('email');"><img src="<?=$templateFolder?>/img/email.jpg" title="<?=GetMessage("skyweb24_loyaltyprogram_SEND_LINK")?>"></a>
			<?}*/?>
		</div>
		...
		<?if(count($arResult['COUPONS'])>0){?>
			...
			<?foreach($arResult['COUPONS'] as $keyCoupon=>$nextCoupon){
				...
				<div class="couponItem">
					...
					<span class="link" onclick="show_url(this);"><?=GetMessage("skyweb24_loyaltyprogram.CLICK_ON_THE_LINK")?></span>
					<span class="link sub_url" style="display:none;"><?=GetMessage("skyweb24_loyaltyprogram.COUPON_IS_COPIED")?></span>
					<?if(!empty($arParams['EDIT_COUPON']) && $arParams['EDIT_COUPON']=='Y' && $nextCoupon['TYPE']=='user_prop'){?>
						<a class="link edit_coupon" onclick="show_edit_coupon(this)" href="javascript:void(0);"><?=GetMessage("skyweb24_loyaltyprogram_EDIT_COUPON")?></a>
						<form name="edit_coupon" style="display:none;">
							...
							<button type="submit"><?=GetMessage("skyweb24_loyaltyprogram_BUTTON_SUBMIT")?></button>
							<button type="button"><?=GetMessage("skyweb24_loyaltyprogram_BUTTON_CANCEL")?></button>
						</form><br>
					<?}?>
					<?if(!empty($nextCoupon['DESCRIPTION'])){?>
						<span class="link" onclick="show_desc(this);" data-hide="<?=GetMessage("skyweb24_loyaltyprogram_CLICK_DESCRIPTION_HIDE")?>" data-show="<?=GetMessage("skyweb24_loyaltyprogram_CLICK_DESCRIPTION_SHOW")?>"><?=GetMessage("skyweb24_loyaltyprogram_CLICK_DESCRIPTION_SHOW")?></span>
		...
		<script>
			...
			var buttonName='<?=Loc::getMessage('skyweb24_loyaltyprogram_SEND')?>';
			var errorMessage='<?=Loc::getMessage('skyweb24_loyaltyprogram_ERRORMESSAGE')?>';
			var successMessage='<?=Loc::getMessage('skyweb24_loyaltyprogram_SUCCESSMESSAGE')?>';
		</script>
		<?}?>
	</div>
