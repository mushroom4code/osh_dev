			...
			if($this->arParams['USER_ID']>0){
				...
				if($isActiveModule=='Y'){
                    ...
                    if(!empty($this->arParams['SHOW_QRCODE']) && $this->arParams['SHOW_QRCODE']=='Y' && !empty($this->arResult['REF_LINK'])){
                        ...
                        $this->arResult['QRCODE_IMG']=Skyweb24\Loyaltyprogram\Tools\Qrcode::show($this->arResult['REF_LINK'], $qrcode_level, $qrcode_size, $qrcode_margin);
                    }
				...
				$this->arResult['STATISTIC']=\Skyweb24\Loyaltyprogram\Statistic::getStatisticByLink($this->arParams['USER_ID']);
				if(!empty($this->arResult['COUPONS'])){
					foreach($this->arResult['COUPONS'] as $nextCoupon){
						if(empty($nextCoupon['COUPON'])){
							continue;
						}
						$this->arResult['STATISTIC']['COUPONS'][$nextCoupon['COUPON']]=\Skyweb24\Loyaltyprogram\Statistic::getStatisticByCoupons([$nextCoupon['COUPON']]);
					}
				};
			}else{
