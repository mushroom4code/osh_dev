...
class Register extends Profile implements Profileinterface{
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_REGISTER'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_REGISTER']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_REGISTER'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_REGISTER',
	...
	public function setBonus($userId){
		if($this->profileSetting['settings']['bonuses']['bonus_size']>0 && $this->ruleCheck()==true){
			...
			$idIns=$DB->Insert($this->globalSettings->getTableBonusList(), [
				...
				'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_REGISTRATION", ["#NUM#"=>$this->profileSetting['settings']['bonuses']['bonus_size']])).'"',
				'email'=>"'".$mailTmplt."'"
			], $err_mess.__LINE__);
		...
		foreach($rewards as $key=>$val){
			if($val>0){
				...
				$idIns=$DB->Insert($this->globalSettings->getTableBonusList(), [
					...
					'date_add'=>'FROM_UNIXTIME('.$startBonus.')',
					'date_remove'=>$endBonus,
					'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_REF_REGISTRATION", ["#NUM#"=>$val])).'"',
					'email'=>"'".$mailTmplt."'"
				], $err_mess.__LINE__);
				if($startBonus==$currentTime){
					Eventmanager::manageBonuses($idIns);
	...
	private function getUserEmail($userId){
		if(!empty($userId)){
			...
			if(!empty($emailSettings['SKYWEB24_LOYAL_REGISTER']['userTemplate'])){
				$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_REGISTER']['userTemplate']);
				if($arEM = $rsEM->Fetch()){
					$sites=array_keys($this->globalSettings->getSites());
					...
					if(!empty($arUser['EMAIL'])){
						return [
							"EVENT_NAME" => "SKYWEB24_LOYAL_REGISTER",
							"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_REGISTER']['userTemplate'],
	...
	private function getRefEmail($userId, $bonus){
		if(!empty($userId)){
			$emailSettings=$this->profileSetting['email_settings'];
			if(!empty($emailSettings['SKYWEB24_LOYAL_REGISTER']['refTemplate'])){
				...
				if(!empty($arUser['EMAIL'])){
					...
					if(!isset($this->refTemplate)){
						$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_REGISTER']['refTemplate']);
						if($arEM = $rsEM->Fetch()){
					...
					if($this->refTemplate!==false){
						return [
							"EVENT_NAME" => "SKYWEB24_LOYAL_REGISTER",
							"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_REGISTER']['refTemplate'],
							"LANGUAGE_ID" => LANGUAGE_ID,
							"LID" => $sites[0],
							"C_FIELDS" => [
								"EMAIL_TO" => $arUser['EMAIL'],
