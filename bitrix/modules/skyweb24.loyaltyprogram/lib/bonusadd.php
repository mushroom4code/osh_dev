...
class Bonusadd extends Profile implements Profileinterface{
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_BONUSADD'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_BONUSADD']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_BONUSADD'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_BONUSADD',
			...
			if($price>0){
				...
				if($bonusAdd>0){
					$idIns=$DB->Insert($this->globalSettings->getTableBonusList(), [
						...
						'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_BONUS_ADD", ["#NUM#"=>$bonusAdd])).'"',
						'email'=>"'".$mailTmplt."'"
					], $err_mess.__LINE__);
	...
	private function getUserEmail($userId, $bonus){
		...
		if(!empty($emailSettings['SKYWEB24_LOYAL_BONUSADD']['userTemplate'])){
			$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_BONUSADD']['userTemplate']);
			if($arEM = $rsEM->Fetch()){
				$sites=array_keys($this->globalSettings->getSites());
				...
				if(!empty($arUser['EMAIL'])){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_BONUSADD",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_BONUSADD']['userTemplate'],
		...
		foreach($rewards as $key=>$val){
			if($val>0){
				...
				$idIns=$DB->Insert($this->globalSettings->getTableBonusList(), [
					...
					'date_add'=>'FROM_UNIXTIME('.$startBonus.')',
					'date_remove'=>$endBonus,
					'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_REF_BONUS_ADD", ["#NUM#"=>$val])).'"',
					'email'=>"'".$mailTmplt."'"
				], $err_mess.__LINE__);
				if($startBonus==$currentTime){
					Eventmanager::manageBonuses($idIns);
	...
	private function getRefEmail($userId, $bonus){
		$emailSettings=$this->profileSetting['email_settings'];
		if(!empty($emailSettings['SKYWEB24_LOYAL_BONUSADD']['refTemplate'])){
			...
			if(!empty($arUser['EMAIL'])){
				...
				if(!isset($this->refTemplate)){
					$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_BONUSADD']['refTemplate']);
					if($arEM = $rsEM->Fetch()){
				...
				if($this->refTemplate!==false){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_BONUSADD",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_BONUSADD']['refTemplate'],
						"LANGUAGE_ID" => LANGUAGE_ID,
						"LID" => $sites[0],
						"C_FIELDS" => [
							"EMAIL_TO" => $arUser['EMAIL'],
