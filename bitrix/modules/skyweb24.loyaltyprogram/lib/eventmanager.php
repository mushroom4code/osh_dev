...
class Eventmanager{
	...
	*/
	public static function manageBonuses($idRow=0){
		...
		while($row = $res->Fetch()){
			...
			if(!empty($row['date_remove'])){
				...
				if($dateEnd->getTimestamp()<$currentTime){
					...
					$comments[]=Loc::getMessage("skyweb24.loyaltyprogram_BONUS_OVERDUE", ["#NUM#"=>Tools::roundBonus($row['bonus'])]);
			...
			if($dateStart->getTimestamp()<=$currentTime){
				...
				$type=strtoupper('skyweb24_loyal_'.$row['profile_type']);
		...
		if(!empty($tmpOptions['notify_before_overdue']) && $tmpOptions['notify_before_overdue']=='Y'){
			...
			$select='select
						skyweb24_loyal_bonuses.id as id,
						skyweb24_loyal_bonuses.bonus as bonus,
						skyweb24_loyal_bonuses.user_id as user_id,
						skyweb24_loyal_bonuses.profile_id as profile_id,
						DATEDIFF(skyweb24_loyal_bonuses.date_remove, now())  as diff,
						skyweb24_loyal_bonuses.date_remove as date_remove,
						skyweb24_loyal_bonuses.date_add as date_add,
						b_user.email as email
					from skyweb24_loyal_bonuses
					left join b_user
					on (b_user.ID=skyweb24_loyal_bonuses.user_id)
					where status="active"
						and date_remove is not null
						and bonus>0
						and notify="N"
						and DATEDIFF(skyweb24_loyal_bonuses.date_remove, now())>0
						and DATEDIFF(skyweb24_loyal_bonuses.date_remove, now())<'.$days.';';
			$res=$DB->Query($select);
			while($row = $res->Fetch()){
				if(!empty($row['email'])){
		...
		$select='select
					...
					skyweb24_loyal_bonuses.user_id as user_id,
					skyweb24_loyal_bonuses.profile_id as profile_id,
					skyweb24_loyal_bonuses.currency as currency,
					DATEDIFF(skyweb24_loyal_bonuses.date_remove, now())  as diff,
					skyweb24_loyal_bonuses.date_remove as date_remove,
					skyweb24_loyal_bonuses.date_add as date_add,
					b_user.email as email
				...
				where status="active"
					and date_remove is not null
					and bonus>0
					and DATEDIFF(skyweb24_loyal_bonuses.date_remove, now())<1;';
			$res=$DB->Query($select);
			while($row = $res->Fetch()){
				...
				$type='SKYWEB24_LOYAL_BONUSOVERDUE';
				...
				\CSaleUserAccount::UpdateAccount(
					...
					$type,
					0,
					Loc::getMessage("skyweb24.loyaltyprogram_BONUS_OVERDUE", ["#NUM#"=>Tools::roundBonus($row['bonus'])])
				);
		...
		$select = 'select * from skyweb24_loyal_bonuses where status = "order_payment_overdue_start"';
		$res = $DB->Query($select);
		while($row = $res->Fetch()){
			if($row['bonus']<0){
				...
				if((float)$cale['CURRENT_BUDGET']>=abs($row['bonus'])&&$cale['LOCKED']=='N'){
					...
					\CSaleUserAccount::UpdateAccount(
                            ...
                            $tmpDebt,
                            $row['currency'],
                            "SKYWEB24_LOYAL_BONUSREFUND_LATER",
                            $row['order_id'],
                            Loc::getMessage("skyweb24.loyaltyprogram_PROGRAM_BONUSREFUND_LATER")
                        );
					$DB->Update($settings->getTableBonusList(),[
		...
		$select = 'select * from skyweb24_loyal_bonuses where status = "not-removed"';
		$res = $DB->Query($select);
		while($row = $res->Fetch()){
			\CSaleUserAccount::UpdateAccount(
                $row['user_id'],
                $row['bonus'],
                $row['currency'],
				"SKYWEB24_LOYAL_GROUPS",//"BONUS_USER_GROUP_BONUSREMOVE",
				'',
				$row['add_comment']
            );
			$DB->Update($settings->getTableBonusList(),[
		...
		return '\Skyweb24\Loyaltyprogram\Eventmanager::manageBonuses();';
	}
	
	/**
	* function for birthday users
	*/
	public static function birthday(){
		...
		return '\Skyweb24\Loyaltyprogram\Eventmanager::birthday();';
	}
	...
	*/
	public static function turnover(){
		...
		return '\Skyweb24\Loyaltyprogram\Eventmanager::turnover();';
	}
	...
	*/
	public static function completedProfile(){
		...
		return '\Skyweb24\Loyaltyprogram\Eventmanager::completedProfile();';
	}
	...
	*/
	public static function turnoverRef(){
		...
		return '\Skyweb24\Loyaltyprogram\Eventmanager::turnoverRef();';
	}
	...
	*/
	public static function soaIntegration($tmpOrder, $arUserResult, $request, $arParams, &$arResult){
		if(\Bitrix\Main\Config\Option::get('skyweb24.loyaltyprogram','ref_insert_to_soa')=='Y'){
		$arResult['JS_DATA']['sw24_loyalty_max_bonus']=0;
			...
			$GLOBALS['APPLICATION']->IncludeComponent("skyweb24:order.ajax.bonus2", "", ['TMP_ORDER'=>$tmpOrder],false);
		}
	}
	...
	*/
	public static function setRanks(){
		Ranks::setRanks();
		return '\Skyweb24\Loyaltyprogram\Eventmanager::setRanks();';
	}
