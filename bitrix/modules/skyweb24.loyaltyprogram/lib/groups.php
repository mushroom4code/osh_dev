...
class Groups{
	...
	function __construct (){
		...
		$this->groupList=[
			'bonusAcc'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_BONUSACC"),
			'bonusRemove'=>Loc::getMessage('skyweb24.loyaltyprogram_GROUPS_BONUSREMOVE')
		];
		$this->profileSetting=[
			'type'=>'Groups',
			'typeRemove'=>'Remove'
	...
	public function getFields(){
		return [
				'bonusAcc'=>[
					[
						'code'=>'bonus_size',
						'name'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_BONUSSIZE"),
						'type'=>'number',
						'required'=>true
					],
					[
						'code'=>'select_user',
						'name'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_SELECTUSERTYPE"),
						'type'=>'radiobutton',
						'list'=>[
							'group'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_USERGROUPS2"),
							'user'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_USERIDS2")
						]
					],
					[
						'code'=>'user_groups',
						'name'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_USERGROUPS"),
						'type'=>'multiselect',
						'list'=>$this->globalSettings->getUserGroups()
					],
					[
						'code'=>'user_ids',
						'name'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_USERIDS"),
						'type'=>'user_select',
					],
					[
						'code'=>'bonus_desc',
						'name'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_BONUSDESC"),
						'type'=>'textarea',
						'required'=>true
					],
					[
						'code'=>'bonus_live',
						'name'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_BONUSLIVE"),
						'type'=>'live_type',
						'list'=>[
				...
				'bonusRemove'=>[
					[
						...
						'name'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_BONUSSIZE_REMOVE"),
						'type'=>'number',
						'required'=>true
					],
					[
						...
						'type'=>'radiobutton',
						'list'=>[
							'group'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_USERGROUPS"),
							'user'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_USERIDS")
						]
					],
					[
					...
					[
						...
						'name'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_BONUSDESC_REMOVE"),
						'type'=>'textarea',
						'required'=>true
					]
				]
	...
	public function doSomething($reg){
		...
		$message = new \CAdminMessage(Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_NOT_SELECT"));
		return $message->Show();
	}
	...
	private function bonusRemove($reg){
		...
		if(!empty((int) $reg['bonus_size_remove']) && !empty($reg['bonus_desc_remove'])){
			...
			if(!empty($reg['user_groups_remove']) && $reg['select_user_remove']=='group'){
				...
				$message = new \CAdminMessage(['MESSAGE'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_ERROR_GROUP_USER"), 'TYPE'=>'ERROR']);
				return $message->Show();
			}
			...
			if($this->notifyGroupBonusRemove=='Y'){
				$mailTmplt=[
					"EVENT_NAME" => "SKYWEB24_LOYAL_GENERAL",
			...
			foreach($usersArr as $keyUser=>$valUser){
				...
				$DB->Insert($this->globalSettings->getTableBonusList(), [
					...
					'add_comment'=>'"'.$DB->ForSql(
							Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_GROUP_BONUSREMOVE",
			...
			$message = new \CAdminMessage(['MESSAGE'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_BONUSREMOVE_SUCCESS"), 'TYPE'=>'OK']);
			return $message->Show();

		}
		$message = new \CAdminMessage(Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_NOT_SELECT_BONUSREMOVE"));
		return $message->Show();
	}
		...
		if(!empty((int) $reg['bonus_size']) && !empty($reg['bonus_desc'])){
			...
			foreach($usersArr as $keyUser=>$valUser){
				...
				$comment=$reg['bonus_size']>0?
					Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_GROUP_BONUSACC", ["#NUM#"=>$reg['bonus_size'], '#COMMENT#'=>$reg['bonus_desc']]):
					Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_GROUP_BONUSWITHDRAW", ["#NUM#"=>$reg['bonus_size'], '#COMMENT#'=>$reg['bonus_desc']]);
				$DB->Insert($this->globalSettings->getTableBonusList(), [
			...
			$message = new \CAdminMessage(['MESSAGE'=>Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_BONUSACC_SUCCESS"), 'TYPE'=>'OK']);
			return $message->Show();

		}
		$message = new \CAdminMessage(Loc::getMessage("skyweb24.loyaltyprogram_GROUPS_NOT_SELECT_BONUSACC"));
		return $message->Show();
	}
