...
class Ordering extends Profile implements Profileinterface{
	...
	protected function checkSMSList(){
		...
		if(empty($this->profileSetting['sms_settings']['SKYWEB24_LOYAL_ORDERING_SMS'])){
			$this->profileSetting['sms_settings']['SKYWEB24_LOYAL_ORDERING_SMS']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function SMSType($type){
		$SMSType = [
			'SKYWEB24_LOYAL_ORDERING_SMS'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_ORDERING_SMS',
				'EVENT_TYPE'  => 'sms',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_BONUSADD"),
				'DESCRIPTION'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_BONUSADD_DESC")
			]
		];
		return $SMSType[$type];
	}
	...
	protected function SMSTemplates($key){
		...
		$smsTemplates = [
			'userTemplate'=>[
				...
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_BONUSADD_USERBONUSADD")
			],
			'refTemplate'=>[
				'EVENT_NAME'=>'SKYWEB24_LOYAL_ORDERING_SMS',
				"ACTIVE" => false,
				"SENDER" => "#DEFAULT_SENDER#",
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_BONUSADD_REFBONUSADD")
			]
		];
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_ORDERING'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_ORDERING']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_ORDERING'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_ORDERING',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSADD"),
				'DESCRIPTION'=>'
					#EMAIL_TO# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSADD_EMAIL_TO").'
					#BONUS# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSADD_BONUS").'
					#USER_ID# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSADD_USER_ID").'
				',
				'SORT'=>500
			]
		];
	...
	protected function mailTemplates($key){
		...
		$mailTemplates = [
			'userTemplate'=>[
				...
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSADD_USERBONUSADD"),
			...
			'refTemplate'=>[
				...
				'EMAIL_FROM'=>'#DEFAULT_EMAIL_FROM#',
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSADD_REFBONUSADD"),
			...
			foreach($this->profileSetting['settings']["condition"]["children"] as $nextO){
				if(!isset($nextO['children']) || $nextO['controlId']=='registerbyParentRef'){
					...
					if($status){
						...
						if(count($skipItems)>0 && $nextO['values']['bonus_unit']=='percent'){
						...
						if($nextO['controlId']=='registerbyRef'){
							...
							if(!empty($params['bonus']) && (float) $params['bonus']>0){
								...
								if($fields['bonus']>0){
									...
									$fields['add_comment']='"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_BONUS_ADD", ["#NUM#"=>$fields['bonus']])).'"';
	...
	private function getRefSMS($userId, $bonus){
		...
		if(!empty($SMSSettings['SKYWEB24_LOYAL_ORDERING_SMS']['refTemplate'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_ORDERING_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_ORDERING_SMS']['refTemplate'],
	...
	private function getUserSMS($userId, $bonus){
		$SMSSettings=$this->profileSetting['sms_settings'];
		if(!empty($SMSSettings['SKYWEB24_LOYAL_ORDERING_SMS']['userTemplate'])){
			$userData=Loyaltyprogram\Tools::getUserData($userId);
			if(!empty($userData['PERSONAL_PHONE'])){
				$tmpPhone=$userData['PERSONAL_PHONE'];
			}elseif(!empty($userData['PERSONAL_MOBILE'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_ORDERING_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_ORDERING_SMS']['userTemplate'],
					"LID" => $this->order->getField('LID'),
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus)
	...
	private function getUserEmail($userId, $bonus){
		...
		if(!empty($emailSettings['SKYWEB24_LOYAL_ORDERING']['userTemplate'])){
			$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_ORDERING']['userTemplate']);
			if($arEM = $rsEM->Fetch()){
				$sites=array_keys($this->globalSettings->getSites());
				...
				if(!empty($arUser['EMAIL'])){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_ORDERING",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_ORDERING']['userTemplate'],
		...
		if(count($newRew)>0 && !empty($nextO['values'])){
			...
			if($fields['bonus']>0){
				...
				foreach($newRew as $nextRew){
					...
					if($fields['bonus']>0){
						$this->setBonus=true;
						$fields['add_comment']='"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_REF_BONUS_ADD", ["#NUM#"=>$fields['bonus']])).'"';
	...
	private function getRefEmail($userId, $bonus){
		$emailSettings=$this->profileSetting['email_settings'];
		if(!empty($emailSettings['SKYWEB24_LOYAL_ORDERING']['refTemplate'])){
			global $DB;
			$results=$DB->Query('select * from b_user where id='.$userId);
			$arUser = $results->Fetch();
			if(!empty($arUser['EMAIL'])){
				...
				if(!isset($this->refTemplate)){
					$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_ORDERING']['refTemplate']);
					if($arEM = $rsEM->Fetch()){
				...
				if($this->refTemplate!==false){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_ORDERING",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_ORDERING']['refTemplate'],
						"LANGUAGE_ID" => LANGUAGE_ID,
						"LID" => $cSite,
						"C_FIELDS" => [
							"USER_ID" => $arUser['ID'],
	...
	*/
	...
	public function getConditions($mode=''){
		...
		for($i=1; $i<=$optns['ref_level']; $i++){
			$refRarentArr[$i]=Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_nLevel", ['#N#'=>$i]);
		}

		$mainParams=[[
            'controlId'=> 'sites',
            'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_sites"),
            'group'=> false,
            'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_sites"),
            'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
            'control'=> [[
                'id'=> 'prefix',
                'type'=> 'prefix',
                'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_siteEqual")
            ], [
                'id'=> 'logic',
                'name'=> 'logic',
                'type'=> 'select',
                'values'=> [
                    'Equal'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_EQUAL"),
                    'Not'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_NOTEQUAL")
                ],
                'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_EQUAL"),
                'defaultValue'=> 'Equal'
            ], [
                'type'=> 'select',
                'multiple'=>'Y',
            ...
            'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_GroupParentRef"),
            'group'=> false,
            'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_GroupParentRef"),
            'showIn'=> ['registerbyParentRef', 'registerbyParentRefSubGrp'],
            'control'=> [[
                'id'=> 'prefix',
                'type'=> 'prefix',
                'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_GroupParentRef")
            ], [
                'id'=> 'logic',
                'name'=> 'logic',
                'type'=> 'select',
            ...
            'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_levelParentRef"),
            'group'=> false,
            'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_levelParentRef"),
            'showIn'=> ['registerbyParentRef', 'registerbyParentRefSubGrp'],
            'control'=> [[
                'id'=> 'prefix',
                'type'=> 'prefix',
                'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_levelParentRef")
            ], [
                'id'=> 'logic',
                'name'=> 'logic',
                'type'=> 'select',
                ...
                'id'=> 'level_parent',
                'name'=> 'level_parent',
                //'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_nLevel", ['#N#'=>1]),
                //'defaultValue'=> '1'
                'first_option'=> '...',
                'defaultText'=> '...',
                'defaultValue'=> ''
            ...
            [
                ...
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_GroupRef"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_Group"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp','registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_Group")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
            ...
            [
                'controlId'=> 'typeLink',
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_typeLink_hint"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_typeLink"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp','registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_typeLinkEqual")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
        ...
        if(count($ranks)>0){
            ...
            $mainParams[]=[
                'controlId'=> 'ranksUser',
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_RankUser_hint"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_RankUser"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp','registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_RankUser")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
            ...
            $mainParams[]=[
                ...
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_RankParent_hint"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_RankParent"),
                'showIn'=> ['registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_RankParent")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
		...
		$params = [
			[
				'controlId'=> 'registerbyRef',
				'group'=> true,
				'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_registerbyRef"),
				'showIn'=> ['CondGroup'],
				'visual'=> [
					'controls'=> ['All', 'True'],
					'values'=> [[
					...
					'logic'=> [[
							...
							'message'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_AND")//and
						],
						[
							'style'=> 'condition-logic-and',
							'message'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_ANDNOT")//and not
						],
						[
							'style'=> 'condition-logic-or',
							'message'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_OR")//or
						],
						[
							'style'=> 'condition-logic-or',
							'message'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_ORNOT")//not or
						]
					]
				],
				'control'=> [
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_registerbyRef"),
					[
						'id'=> 'bonus',
						'name'=> 'bonus',
						'type'=> 'input',
					...
					[
						...
						'type'=> 'select',
						'values'=> [
							'percent'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_PERCENT"),
							'bonus'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_BONUS")
						],
						'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_PERCENT"),
					...
					[
						...
						'type'=> 'select',
						'values'=> [
							'Y'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_maxbonus"),
							'N'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_UNLIMITED_BONUS")
						],
						'defaultValue'=> 'Y',
					],
					[
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_BONUS"),
					[
						'id'=>'through_time',
						'name'=>'through_time',
						'type'=> 'select',
						'values'=> [
							'Y'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_through"),
							'N'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_UNLIMITED_THROUGH")
						],
						'defaultValue'=> 'Y',
					],
					[
					...
					[
						...
						'type'=> 'select',
						'values'=> [
							'hour'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_HOUR"),
						...
						'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_DAY"),
						'defaultValue'=> 'day',
					],
					[
						'id'=>'limit_time',
						...
						'type'=> 'select',
						'values'=> [
							'Y'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_lengthOn"),
							'N'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_UNLIMITED_TIME")
						],
						'defaultValue'=> 'Y',
					],
					[
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_round"),
					[
						'id'=> 'bonus_round',
						'name'=> 'bonus_round',
						'type'=> 'select',
						'values'=> [
							'none'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ROUND_NONE"),
							'more'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ROUND_MORE"),
							'less'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ROUND_LESS"),
							'auto'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ROUND_AUTO")
						],
						'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ROUND_NONE"),
						'defaultValue'=> 'none',
					],
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_forWhich"),
					[
						'id'=> 'All',
						'name'=> 'aggregator',
						'type'=> 'select',
						'values'=> [
							'AND'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ALL_CONDITIONS"),
							'OR'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ANY_CONDITIONS"),
						],
						'defaultText'=> '...',
						'defaultValue'=> 'AND',
						'first_option'=> '...'
					...
					[
						...
						'type'=> 'select',
						'values'=> [
							'True'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ANY_EXECUTE"),
							'False'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ANY_NOTEXECUTE")
						],
						'defaultText'=> '...',
						'defaultValue'=> 'True',
						'first_option'=> '...'
				...
				'mess'=> [
					'ADD_CONTROL'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_ADD_CONTROL"),
					'SELECT_CONTROL'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_SELECT_CONTROL")
				]
			],
			[
				'controlId'=> 'registerbyRefSubGrp',
				'group'=> true,
				'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_GROUP_CONTROL"),
				'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_SELECT_VALUE"),
				...
				'visual'=> [
					...
					'logic'=> [[
							'style'=> 'condition-logic-and',
							'message'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_AND")
						],
						[
							'style'=> 'condition-logic-and',
							'message'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_ANDNOT")
						],
						[
							'style'=> 'condition-logic-or',
							'message'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_OR")
						],
						[
							'style'=> 'condition-logic-or',
							'message'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_ORNOT")
						]
					]
				],
				'control'=> [[
			...
			[
				...
				'group'=> true,
				'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_BonusParentRef"),
				'showIn'=> ['CondGroup'],
				'visual'=> [
					'controls'=> ['All', 'True'],
					'values'=> [[
				...
				'control'=> [
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_BonusParentRef"),
					[
						'id'=> 'bonus',
						'name'=> 'bonus',
						'type'=> 'input',
			...
			[
				'controlgroup'=> true,
				'group'=> false,
				'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_MainParameters"),
				'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
				'children'=> $mainParams
			],
			[
				'controlgroup'=> true,
				'group'=> false,
				'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ShopParameters"),
				'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
				'children'=> [[
						'controlId' => 'orderPriceWithoutDiscount',
						'description' => Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_order_price_without_discount"),
						'group' => false,
						'label' => Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ORDER_PRICE_WITHOUT_DISCOUNT"),
						'showIn' => ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
						'control' => [[
							'id' => 'prefix',
							'type' => 'prefix',
							'text' => Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ORDER_PRICE_WITHOUT_DISCOUNT")
						], [
							'id' => 'logic',
							'name' => 'logic',
							'type' => 'select',
							'values' => [
								'more' => Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_MORE"),
								'less' => Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_LESS")
							],
							'defaultText' => Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_MORE"),
							'defaultValue' => 'more'
						],
							[
								'type' => 'input',
					...
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_SKIP_hint_bonus_pay"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_SKIP_bonus_pay"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_SKIP_bonus_pay")
					],[
						'type'=> 'select',
						'multiple'=>'Y',
						'values'=> [
							'yes'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_YES")
						],
						'id'=> 'skip_bonus_pay',
						'name'=> 'skip_bonus_pay',
						'first_option'=> '...',
						'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_YES"),
						'defaultValue'=> 'yes'
					]]
				],[
					'controlId'=> 'skipDiscountProduct',
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_SKIP_hint_discount_product"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_SKIP_DISCOUNT_PRODUCT"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_SKIP_DISCOUNT_PRODUCT")
					],[
						'type'=> 'select',
						'multiple'=>'Y',
						'values'=> [
					...
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_hint_skip_price_product"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_SKIP_PRICE_PRODUCT"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_SKIP_PRICE_PRODUCT")
					],
					[
						'id'=> 'logic',
						'name'=> 'logic',
					...
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_hint_countOrders"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_countOrders"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_countOrders")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'controlId'=> 'product',
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_product"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PRODUCT"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PRODUCT")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_product_cat"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PRODUCT_CAT"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PRODUCT_CAT")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_product_iblock"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PRODUCT_IBLOCK"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PRODUCT_IBLOCK")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'controlId'=> 'discount',
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_discount"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_DISCOUNT"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_DISCOUNT")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_order_price"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ORDER_PRICE"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ORDER_PRICE")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_orderstatuses"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ORDERSTATUSES"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ORDERSTATUSES")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_paysystems"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PAYSYSTEMS"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PAYSYSTEMS")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'controlId'=> 'delyvery',
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_delyvery"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_DELYVERY"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_DELYVERY")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ordering_persontypes"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PERSONTYPES"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_PERSONTYPES")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
					...
					'controlId'=> 'turnover',
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ORDER_ordering_turnover"),
					'group'=> false,
					'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ORDER_TURNOVER"),
					'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
					'control'=> [[
						'id'=> 'prefix',
						'type'=> 'prefix',
						'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_ORDER_TURNOVER")
					], [
						'id'=> 'logic',
						'name'=> 'logic',
						'type'=> 'select',
			...
			[
				...
				'defaultText'=> '',
				'showIn'=> [],
				'control'=> [Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_PERFORM_OPERATIONS")]
			]*/
		];
