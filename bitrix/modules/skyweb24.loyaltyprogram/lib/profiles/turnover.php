...
class Turnover extends Profile implements Profileinterface{
	...
	protected function checkSMSList(){
		...
		if(empty($this->profileSetting['sms_settings']['SKYWEB24_LOYAL_TURNOVER_SMS'])){
			$this->profileSetting['sms_settings']['SKYWEB24_LOYAL_TURNOVER_SMS']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function SMSType($type){
		$SMSType = [
			'SKYWEB24_LOYAL_TURNOVER_SMS'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_TURNOVER_SMS',
				'EVENT_TYPE'  => 'sms',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_TURNOVER"),
				'DESCRIPTION'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_TURNOVER_DESC")
			]
		];
		return $SMSType[$type];
	}
	...
	protected function SMSTemplates($key){
		...
		$smsTemplates = [
			'userTemplate'=>[
				...
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_TURNOVER_USERBONUSADD")
			],
			'refTemplate'=>[
				'EVENT_NAME'=>'SKYWEB24_LOYAL_TURNOVER_SMS',
				"ACTIVE" => false,
				"SENDER" => "#DEFAULT_SENDER#",
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_TURNOVER_REFBONUSADD")
			]
		];
	...
	private function getRefSMS($userId, $bonus, $turnover){
		...
		if(!empty($SMSSettings['SKYWEB24_LOYAL_TURNOVER_SMS']['refTemplate'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_TURNOVER_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_TURNOVER_SMS']['refTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus),
	...
	private function getUserSMS($userId, $bonus, $turnover){
		$SMSSettings=$this->profileSetting['sms_settings'];
		if(!empty($SMSSettings['SKYWEB24_LOYAL_TURNOVER_SMS']['userTemplate'])){
			$userData=Loyaltyprogram\Tools::getUserData($userId);
			if(!empty($userData['PERSONAL_PHONE'])){
				$tmpPhone=$userData['PERSONAL_PHONE'];
			}elseif(!empty($userData['PERSONAL_MOBILE'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_TURNOVER_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_TURNOVER_SMS']['userTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus),
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_TURNOVER'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_TURNOVER']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_TURNOVER'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_TURNOVER',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSTURNOVER"),
				'DESCRIPTION'=>'
					#EMAIL_TO# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSTURNOVER_EMAIL_TO").'
					#BONUS# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSTURNOVER_BONUS").'
					#TURNOVER# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSTURNOVER_TURNOVER").'
				',
				'SORT'=>500
			]
		];
	...
	protected function mailTemplates($key){
		...
		$mailTemplates = [
			'userTemplate'=>[
				...
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSTURNOVER_USERBONUSADD"),
			...
			'refTemplate'=>[
				...
				'EMAIL_FROM'=>'#DEFAULT_EMAIL_FROM#',
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSTURNOVER_REFBONUSADD"),
				...
				foreach($totalUserPeriod as $keyPeriod=>$nextUser){
					...
					foreach($nextUser['users'] as $keyUser=>$bonusUser){
						...
						$fields=[
							...
							'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_BONUSTURNOVER_ADD", ["#NUM#"=>$bonusUser])).'"',
							'email'=>"'".$mailTmplt."'",
							'sms'=>"'".$SMSTmplt."'"
						];
	...
	private function getUserEmail($userId, $bonus, $turnover){
		...
		if(!empty($emailSettings['SKYWEB24_LOYAL_TURNOVER']['userTemplate'])){
			$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_TURNOVER']['userTemplate']);
			if($arEM = $rsEM->Fetch()){
				$sites=array_keys($this->globalSettings->getSites());
				global $DB;
				...
				if(!empty($arUser['EMAIL'])){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_TURNOVER",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_TURNOVER']['userTemplate'],
	...
	private function setReferalBonuses($userId, $refUserId, $bonus, $turnover){
		...
		$fields=[
			...
			'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_BONUSTURNOVERREF_ADD", ["#NUM#"=>$bonus['bonus']])).'"',
			'email'=>"'".$mailTmplt."'"
		];
	...
	private function getRefEmail($userId, $bonus, $turnover){
		$emailSettings=$this->profileSetting['email_settings'];
		if(!empty($emailSettings['SKYWEB24_LOYAL_TURNOVER']['refTemplate'])){
			global $DB;
			$results=$DB->Query('select * from b_user where id='.$userId);
			$arUser = $results->Fetch();
			if(!empty($arUser['EMAIL'])){
				...
				if(!isset($this->refTemplate)){
					$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_TURNOVER']['refTemplate']);
					if($arEM = $rsEM->Fetch()){
				...
				if($this->refTemplate!==false){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_TURNOVER",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_TURNOVER']['refTemplate'],
						"LANGUAGE_ID" => LANGUAGE_ID,
						"LID" => $sites[0],
						"C_FIELDS" => [
							"EMAIL_TO" => $arUser['EMAIL'],
	...
	*/
	...
	public function getConditions($mode=''){
		...
		$mainParams=[[
            'controlId'=> 'sites',
            'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_turn_sites"),
            'group'=> false,
            ...
            [
                ...
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_turn_GroupRef"),
                'group'=> false,
                ...
                'controlId'=> 'turnover',
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_hint_turnoverPeriod"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_turnoverPeriod"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_turnoverPeriod")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
                ...
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_hint_turnoverBonusAdded"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_turnoverBonusAdded"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_turnoverBonusAdded")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
                ...
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_hint_turnoverBonusRemove"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_turnoverBonusRemove"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_turnoverBonusRemove")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
                ...
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_hint_levelParentRef"),
                'group'=> false,
			...
			[
				...
				'control'=> [
					...
					[
						...
						'type'=> 'select',
						'values'=> [
							'percent'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_PERCENT_TURNOVER"),
							'bonus'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_BONUS")
						],
						'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_PERCENT_TURNOVER"),
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_BY"),
					[
						'type'=> 'select',
						'values'=>[
							'week'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_WEEK"),
						...
						'name'=> 'period',
						'first_option'=> '...',
						'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_MONTH"),
						'defaultValue'=> 'month'
					],
			...
			[
				...
				'control'=> [
					...
					[
						...
						'type'=> 'select',
						'values'=> [
							'percent'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_PERCENT_TURNOVER_USER"),
							'bonus'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_BONUS")
						],
						'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_PERCENT_TURNOVER_USER"),
						'defaultValue'=> 'percent',
					],
					[
						'id'=>'allow_bonus_max',
