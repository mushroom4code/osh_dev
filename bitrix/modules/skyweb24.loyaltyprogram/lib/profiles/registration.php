...
class Registration extends Profile implements Profileinterface{
	...
	protected function checkSMSList(){
		...
		if(empty($this->profileSetting['sms_settings']['SKYWEB24_LOYAL_REGISTRATION_SMS'])){
			$this->profileSetting['sms_settings']['SKYWEB24_LOYAL_REGISTRATION_SMS']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function SMSType($type){
		$SMSType = [
			'SKYWEB24_LOYAL_REGISTRATION_SMS'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_REGISTRATION_SMS',
				'EVENT_TYPE'  => 'sms',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_REGISTRATION"),
				'DESCRIPTION'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_REGISTRATION_DESC")
			]
		];
		return $SMSType[$type];
	}
	...
	protected function SMSTemplates($key){
		...
		$smsTemplates = [
			'userTemplate'=>[
				...
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_REGISTRATION_USERBONUSADD")
			],
			'refTemplate'=>[
				'EVENT_NAME'=>'SKYWEB24_LOYAL_REGISTRATION_SMS',
				"ACTIVE" => false,
				"SENDER" => "#DEFAULT_SENDER#",
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_REGISTRATION_REFBONUSADD")
			]
		];
	...
	private function getRefSMS($userId, $bonus){
		...
		if(!empty($SMSSettings['SKYWEB24_LOYAL_REGISTRATION_SMS']['refTemplate'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_REGISTRATION_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_REGISTRATION_SMS']['refTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus)
	...
	private function getUserSMS($userId, $bonus){
		$SMSSettings=$this->profileSetting['sms_settings'];
		if(!empty($SMSSettings['SKYWEB24_LOYAL_REGISTRATION_SMS']['userTemplate'])){
			$userData=Loyaltyprogram\Tools::getUserData($userId);
			if(!empty($userData['PERSONAL_PHONE'])){
				$tmpPhone=$userData['PERSONAL_PHONE'];
			}elseif(!empty($userData['PERSONAL_MOBILE'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_REGISTRATION_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_REGISTRATION_SMS']['userTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus)
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_REGISTRATION'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_REGISTRATION']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_REGISTRATION'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_REGISTRATION',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_EVENT_REGISTER"),
				'DESCRIPTION'=>'
					#EMAIL_TO# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_REGISTER_EMAIL_TO").'
					#BONUS# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_REGISTER_BONUS").'
				',
				'SORT'=>500
			]
		];
	...
	protected function mailTemplates($key){
		...
		$mailTemplates = [
			'userTemplate'=>[
				...
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_REGISTER_USERREGISTER"),
			...
			'refTemplate'=>[
				...
				'EMAIL_FROM'=>'#DEFAULT_EMAIL_FROM#',
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_REGISTER_REFREGISTER"),
						...
						if($nextO['controlId']=='registerbyRef'){
							if(!empty($nextO['values']['bonus'])){
								...
								$fields=[
									...
									'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_REGISTRATION", ["#NUM#"=>$nextO['values']['bonus']])).'"',
									'email'=>"'".$mailTmplt."'",
									'sms'=>"'".$SMSTmplt."'"
								];
								//action_id
			...
			foreach($newRew as $nextRew){
				//if(in_array($i, $newRew)){
					...
					$fields=[
						...
						'date_add'=>'FROM_UNIXTIME('.$startBonus.')',
						'date_remove'=>$endBonus,
						'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_REF_REGISTRATION", ["#NUM#"=>$nextO['values']['bonus']])).'"',
						'email'=>"'".$mailTmplt."'",
						'sms'=>"'".$SMSTmplt."'"
					];
					//action_id
	...
	private function getUserEmail($userId, $bonus){
		if(!empty($userId)){
			...
			if(!empty($emailSettings['SKYWEB24_LOYAL_REGISTRATION']['userTemplate'])){
				$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_REGISTRATION']['userTemplate']);
				if($arEM = $rsEM->Fetch()){
					$sites=array_keys($this->globalSettings->getSites());				
					...
					if(!empty($arUser['EMAIL'])){
						return [
							"EVENT_NAME" => "SKYWEB24_LOYAL_REGISTRATION",
							"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_REGISTRATION']['userTemplate'],
	...
	private function getRefEmail($userId, $bonus){
		if(!empty($userId)){
			global $DB;
			$emailSettings=$this->profileSetting['email_settings'];
			if(!empty($emailSettings['SKYWEB24_LOYAL_REGISTRATION']['refTemplate'])){
				$results=$DB->Query('select * from b_user where id='.$userId);
				$arUser = $results->Fetch();
				...
				if(!empty($arUser['EMAIL'])){
					...
					if(!isset($this->refTemplate)){
						$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_REGISTRATION']['refTemplate']);
						if($arEM = $rsEM->Fetch()){
					...
					if($this->refTemplate!==false){
						return [
							"EVENT_NAME" => "SKYWEB24_LOYAL_REGISTRATION",
							"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_REGISTRATION']['refTemplate'],
							"LANGUAGE_ID" => LANGUAGE_ID,
							"LID" => $sites[0],
							"C_FIELDS" => [
								"EMAIL_TO" => $arUser['EMAIL'],
	...
	public function getConditions($mode=''){
		...
		$mainParams=[[
            ...
            'control'=> [[
                ...
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_typeLinkEqual_hint"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_typeLink"),
                ...
                'controlId'=> 'sites',
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_sites_hint"),
                'group'=> false,
		...
		$params = [
			[
				...
				'control'=> [
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_bonusbyRegister"),
					[
						'id'=>'through_time',
						'name'=>'through_time',
						'type'=> 'select',
			...
			[
				...
				'control'=> [
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_BonusParentRefByRegRef"),
					[
						'id'=>'through_time',
						'name'=>'through_time',
						'type'=> 'select',
			...
			[
				...
				'children'=> [[
					'controlId'=> 'discount',
					'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_DISCOUNT_hint"),
					'group'=> false,
