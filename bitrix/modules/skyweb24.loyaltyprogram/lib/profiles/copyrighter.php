...
class Copyrighter extends Profile implements Profileinterface{
						...
						if(!empty($params['bonus']) && (int) $params['bonus']>0){
							foreach($cCopyrightProducts as $cProduct=>$cUser){
								...
								if($fields['bonus']>0){
									...
									$fields['add_comment']='"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_COPYRIGHT_BONUS_ADD", ["#NUM#"=>$fields['bonus'], '#PRODUCT_ID#'=>$cProduct])).'"';
	...
	*/
	...
	protected function checkSMSList(){
		...
		if(empty($this->profileSetting['sms_settings']['SKYWEB24_LOYAL_COPYRIGHTER_SMS'])){
			$this->profileSetting['sms_settings']['SKYWEB24_LOYAL_COPYRIGHTER_SMS']=[
				'copyrighter'=>0
			];
		}
		$this->checkSMSMain();
	...
	protected function SMSType($type){
		$SMSType = [
			'SKYWEB24_LOYAL_COPYRIGHTER_SMS'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_COPYRIGHTER_SMS',
				'EVENT_TYPE'  => 'sms',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_BONUSCOPYRIGHTADD"),
				'DESCRIPTION'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_BONUSCOPYRIGHTADD_DESC")
			]
		];
		return $SMSType[$type];
	}
	...
	protected function SMSTemplates($key){
		...
		$smsTemplates = [
			'copyrighter'=>[
				...
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_BONUSCOPYRIGHTADD_USERBONUSADD")
			]
		];
	...
	private function getUserSMS($userId, $bonus){
		...
		if(!empty($SMSSettings['SKYWEB24_LOYAL_COPYRIGHTER_SMS']['copyrighter'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_COPYRIGHTER_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_COPYRIGHTER_SMS']['copyrighter'],
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_COPYRIGHTER'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_COPYRIGHTER']=[
				'copyrighter'=>0
			];
		}
		$this->checkEmailMain();
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_COPYRIGHTER'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_COPYRIGHTER',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSCOPYRIGHTADD"),
				'DESCRIPTION'=>'
					#EMAIL_TO# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSCOPYRIGHTADD_EMAIL_TO").'
					#BONUS# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSCOPYRIGHTADD_BONUS").'
					#USER_ID# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSCOPYRIGHTADD_USER_ID").'
					#PRODUCT_ID# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSCOPYRIGHTADD_PRODUCT_ID").'
					#PRODUCT_NAME# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSCOPYRIGHTADD_PRODUCT_NAME").'
				',
				'SORT'=>500
			]
		];
	...
	protected function mailTemplates($key){
		...
		$mailTemplates = [
			'copyrighter'=>[
				...
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BONUSCOPYRIGHTADD_USERBONUSADD"),
	...
	private function getUserEmail($userId, $bonus, $idProduct){
		...
		if(!empty($emailSettings['SKYWEB24_LOYAL_COPYRIGHTER']['copyrighter'])){
			$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_COPYRIGHTER']['copyrighter']);
			if($arEM = $rsEM->Fetch()){
				$sites=array_keys($this->globalSettings->getSites());
				global $DB;
				...
				if(!empty($arUser['EMAIL'])){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_COPYRIGHTER",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_COPYRIGHTER']['copyrighter'],
	...
	*/
	...
	public function getConditions($mode=''){
        ...
        $mainParams=[[
            ...
            'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_copyright_GroupRef"),
            'group'=> false,
            'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_GroupCopyright"),
            'showIn'=> ['copyrighter', 'copyrighterSubGrp'],
            'control'=> [[
                'id'=> 'prefix',
                'type'=> 'prefix',
                'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_GroupCopyright")
            ], [
                'id'=> 'logic',
                'name'=> 'logic',
                'type'=> 'select',
		...
		$params = [
			[
				'controlId'=> 'copyrighter',
				'group'=> true,
				'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_registerbyCopyrighter"),
				'showIn'=> ['CondGroup'],
				'visual'=> [
					'controls'=> ['All', 'True'],
					'values'=> [[
				...
				'control'=> [
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_registerbyCopyrighter"),
					[
						'id'=> 'bonus',
						'name'=> 'bonus',
						'type'=> 'input',
