...
class Subscribe extends Profile implements Profileinterface{
	...
	protected function checkSMSList(){
		...
		if(empty($this->profileSetting['sms_settings']['SKYWEB24_LOYAL_SUBSCRIBE_SMS'])){
			$this->profileSetting['sms_settings']['SKYWEB24_LOYAL_SUBSCRIBE_SMS']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function SMSType($type){
		$SMSType = [
			'SKYWEB24_LOYAL_SUBSCRIBE_SMS'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_SUBSCRIBE_SMS',
				'EVENT_TYPE'  => 'sms',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_SUBSCRIBE"),
				'DESCRIPTION'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_SUBSCRIBE_DESC")
			]
		];
		return $SMSType[$type];
	}
	...
	protected function SMSTemplates($key){
		...
		$smsTemplates = [
			'userTemplate'=>[
				...
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_SUBSCRIBE_USERBONUSADD")
			],
			'refTemplate'=>[
				'EVENT_NAME'=>'SKYWEB24_LOYAL_SUBSCRIBE_SMS',
				"ACTIVE" => false,
				"SENDER" => "#DEFAULT_SENDER#",
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_SUBSCRIBE_REFBONUSADD")
			]
		];
	...
	private function getRefSMS($userId, $bonus){
		...
		if(!empty($SMSSettings['SKYWEB24_LOYAL_SUBSCRIBE_SMS']['refTemplate'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_SUBSCRIBE_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_SUBSCRIBE_SMS']['refTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus)
	...
	private function getUserSMS($userId, $bonus){
		$SMSSettings=$this->profileSetting['sms_settings'];
		if(!empty($SMSSettings['SKYWEB24_LOYAL_SUBSCRIBE_SMS']['userTemplate'])){
			$userData=Loyaltyprogram\Tools::getUserData($userId);
			if(!empty($userData['PERSONAL_PHONE'])){
				$tmpPhone=$userData['PERSONAL_PHONE'];
			}elseif(!empty($userData['PERSONAL_MOBILE'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_SUBSCRIBE_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_SUBSCRIBE_SMS']['userTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus)
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_SUBSCRIBE'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_SUBSCRIBE']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_SUBSCRIBE'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_SUBSCRIBE',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SUBSCRIBE"),
				'DESCRIPTION'=>'
					#EMAIL_TO# - '.Loc::getMessage("skyweb24.loyaltyprogram_SUBSCRIBE_EMAIL_TO").'
					#BONUS# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SUBSCRIBE_BONUS").'
					#SUBSCRIBE_NAME# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SUBSCRIBE_SUBSCRIBENAME").'
				',
				'SORT'=>500
			]
		];
	...
	protected function mailTemplates($key){
		...
		$mailTemplates = [
			'userTemplate'=>[
				...
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SUBSCRIBE_USERBONUSADD"),
			...
			'refTemplate'=>[
				...
				'EMAIL_FROM'=>'#DEFAULT_EMAIL_FROM#',
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SUBSCRIBE_REFBONUSADD"),
	...
	private function getUserEmail($userId, $bonus, $subscribe){
		...
		if(!empty($emailSettings['SKYWEB24_LOYAL_SUBSCRIBE']['userTemplate'])){
			$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_SUBSCRIBE']['userTemplate']);
			if($arEM = $rsEM->Fetch()){
				$sites=array_keys($this->globalSettings->getSites());
				...
				if(!empty($arUser['EMAIL'])){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_SUBSCRIBE",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_SUBSCRIBE']['userTemplate'],
	...
	private function getRefEmail($userId, $bonus, $subscribe){	
		$emailSettings=$this->profileSetting['email_settings'];
		if(!empty($emailSettings['SKYWEB24_LOYAL_SUBSCRIBE']['refTemplate'])){
			global $DB;
			$results=$DB->Query('select * from b_user where id='.$userId);
			$arUser = $results->Fetch();
			if(!empty($arUser['EMAIL'])){
				...
				if(!isset($this->refTemplate)){
					$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_SUBSCRIBE']['refTemplate']);
					if($arEM = $rsEM->Fetch()){
				...
				if($this->refTemplate!==false){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_SUBSCRIBE",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_SUBSCRIBE']['refTemplate'],
						"LANGUAGE_ID" => LANGUAGE_ID,
						"LID" => $sites[0],
						"C_FIELDS" => [
							"EMAIL_TO" => $arUser['EMAIL'],
	...
	public function getConditions($mode=''){
		...
		$mainParams=[[
            ...
            [
                'controlId'=> 'subscribe',
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_subscribe_desc"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_subscribe"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_subscribe")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
	...
	public function setBonus($event=false){
		if(\Bitrix\Main\Loader::includeModule('sender') && !empty($this->profileSetting['settings']["condition"]["children"]) && $event!==false){
			...
			//for test!!
			//$fields['fields']['CODE']='skyweb24@yandex.ru';
			global $DB;
									...
									if($status){
										if($nextO['controlId']=='registerbyRef'){
											...
											$insertArr=[
												...
												'status'=>'"inactive"',
												'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_SUBSCRIBE", ["#NUM#"=>$params['bonus']])).'"',
												'email'=>"'".$mailTmplt."'",
												'sms'=>"'".$SMSTmplt."'"
											];
	...
	private function setReferalBonuses($nextO){
		...
		if((int) $params['bonus']>0 && count($newRew)>0){
			...
			foreach($newRew as $nextRew){
				...
				$insertArr['add_comment']='"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_REF_SUBSCRIBE", ["#NUM#"=>$params['bonus']])).'"';
