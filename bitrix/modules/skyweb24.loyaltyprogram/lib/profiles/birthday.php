...
class Birthday extends Profile implements Profileinterface{
	...
	protected function checkSMSList(){
		...
		if(empty($this->profileSetting['sms_settings']['SKYWEB24_LOYAL_BIRTHDAY_SMS'])){
			$this->profileSetting['sms_settings']['SKYWEB24_LOYAL_BIRTHDAY_SMS']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function SMSType($type){
		$SMSType = [
			'SKYWEB24_LOYAL_BIRTHDAY_SMS'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_BIRTHDAY_SMS',
				'EVENT_TYPE'  => 'sms',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_BIRTHDAY"),
				'DESCRIPTION'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_BIRTHDAY_DESC")
			]
		];
		return $SMSType[$type];
	}
	...
	protected function SMSTemplates($key){
		...
		$smsTemplates = [
			'userTemplate'=>[
				...
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_BIRTHDAY_USERBONUSADD")
			],
			'refTemplate'=>[
				'EVENT_NAME'=>'SKYWEB24_LOYAL_BIRTHDAY_SMS',
				"ACTIVE" => false,
				"SENDER" => "#DEFAULT_SENDER#",
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_BIRTHDAY_REFBONUSADD")
			]
		];
	...
	private function getRefSMS($userId, $bonus){
		...
		if(!empty($SMSSettings['SKYWEB24_LOYAL_BIRTHDAY_SMS']['refTemplate'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_BIRTHDAY_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_BIRTHDAY_SMS']['refTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus)
	...
	private function getUserSMS($userId, $bonus){
		$SMSSettings=$this->profileSetting['sms_settings'];
		if(!empty($SMSSettings['SKYWEB24_LOYAL_BIRTHDAY_SMS']['userTemplate'])){
			$userData=Loyaltyprogram\Tools::getUserData($userId);
			if(!empty($userData['PERSONAL_PHONE'])){
				$tmpPhone=$userData['PERSONAL_PHONE'];
			}elseif(!empty($userData['PERSONAL_MOBILE'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_BIRTHDAY_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_BIRTHDAY_SMS']['userTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus)
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_BIRTHDAY'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_BIRTHDAY']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_BIRTHDAY'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_BIRTHDAY',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BIRTHDAY"),
				'DESCRIPTION'=>'
					#EMAIL_TO# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BIRTHDAY_EMAIL_TO").'
					#BONUS# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BIRTHDAY_BONUS").'
				',
				'SORT'=>500
			]
		];
	...
	protected function mailTemplates($key){
		...
		$mailTemplates = [
			'userTemplate'=>[
				...
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BIRTHDAY_USERBIRTHDAY"),
			...
			'refTemplate'=>[
				...
				'EMAIL_FROM'=>'#DEFAULT_EMAIL_FROM#',
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_BIRTHDAY_REFUSERBIRTHDAY"),
	...
	*/
									...
									if($nextO['controlId']=='registerbyRef'){
										...
										if($isDay){
											...
											$insertArr=[
												...
												'status'=>'"inactive"',
												'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_BIRTHDAY", ["#NUM#"=>$params['bonus']])).'"',
												'email'=>"'".$mailTmplt."'",
												'sms'=>"'".$SMSTmplt."'"
											];
	...
	private function getUserEmail($userId, $bonus){
		...
		if(!empty($emailSettings['SKYWEB24_LOYAL_BIRTHDAY']['userTemplate'])){
			$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_BIRTHDAY']['userTemplate']);
			if($arEM = $rsEM->Fetch()){
				$sites=array_keys($this->globalSettings->getSites());
				...
				if(!empty($arUser['EMAIL'])){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_BIRTHDAY",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_BIRTHDAY']['userTemplate'],
		...
		if((int) $params['bonus']>0 && count($newRew)>0){
			...
			if($isDay){
				...
				$insertArr=[
					...
					'profile_id'=>$this->profileSetting['id'],
					'status'=>'"inactive"',
					'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_REF_BIRTHDAY", ["#NUM#"=>$params['bonus']])).'"',
				];
				...
				foreach($newRew as $nextRew){
					...
					$insertArr['add_comment']='"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_REF_BIRTHDAY", ["#NUM#"=>$params['bonus']])).'"';
	...
	private function getRefEmail($userId, $bonus){	
		$emailSettings=$this->profileSetting['email_settings'];
		if(!empty($emailSettings['SKYWEB24_LOYAL_BIRTHDAY']['refTemplate'])){
			global $DB;
			$results=$DB->Query('select * from b_user where id='.$userId);
			$arUser = $results->Fetch();
			if(!empty($arUser['EMAIL'])){
				...
				if(!isset($this->refTemplate)){
					$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_BIRTHDAY']['refTemplate']);
					if($arEM = $rsEM->Fetch()){
				...
				if($this->refTemplate!==false){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_BIRTHDAY",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_BIRTHDAY']['refTemplate'],
						"LANGUAGE_ID" => LANGUAGE_ID,
						"LID" => $sites[0],
						"C_FIELDS" => [
							"EMAIL_TO" => $arUser['EMAIL'],
	...
	*/
	...
	public function getConditions($mode=''){
		...
		$mainParams=[[
            ...
            [
                'controlId'=> 'age',
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_birthday_age"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_age"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_age")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
                    ...
                    'show_value'=>'Y',
                    'defaultValue'=>'',
                    'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_UNLIMITED_AGE"),
                ]]
            ],
            [
		...
		$params = [
			[
				...
				'control'=> [
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_bonusbyBitrhDay"),
					//Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_UNLIMITED_BEFORE"),
					[
						'id'=>'before_date',
						'name'=>'before_date',
						'type'=> 'select',
						'values'=> [
							'Y'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_UNLIMITED_BEFORE"),
			...
			[
				...
				'control'=> [
					...
					Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_BonusParentRefByBirthDay"),
					[
						'id'=>'before_date',
						'name'=>'before_date',
						'type'=> 'select',
