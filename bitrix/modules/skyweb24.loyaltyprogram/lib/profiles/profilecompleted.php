...
class Profilecompleted extends Profile implements Profileinterface{
	...
	protected function checkSMSList(){
		...
		if(empty($this->profileSetting['sms_settings']['SKYWEB24_LOYAL_PROFILECOMPLETED_SMS'])){
			$this->profileSetting['sms_settings']['SKYWEB24_LOYAL_PROFILECOMPLETED_SMS']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function SMSType($type){
		$SMSType = [
			'SKYWEB24_LOYAL_PROFILECOMPLETED_SMS'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_PROFILECOMPLETED_SMS',
				'EVENT_TYPE'  => 'sms',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_PROFILECOMPLETED"),
				'DESCRIPTION'=>Loc::getMessage("skyweb24.loyaltyprogram_SMS_PROFILECOMPLETED_DESC")
			]
		];
		return $SMSType[$type];
	}
	...
	protected function SMSTemplates($key){
		...
		$smsTemplates = [
			'userTemplate'=>[
				...
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_PROFILECOMPLETED_USERBONUSADD")
			],
			'refTemplate'=>[
				'EVENT_NAME'=>'SKYWEB24_LOYAL_PROFILECOMPLETED_SMS',
				"ACTIVE" => false,
				"SENDER" => "#DEFAULT_SENDER#",
				"RECEIVER" => "#USER_PHONE#",
				"MESSAGE" => GetMessage("skyweb24.loyaltyprogram_SMS_PROFILECOMPLETED_REFBONUSADD")
			]
		];
	...
	private function getRefSMS($userId, $bonus, $percent){
		...
		if(!empty($SMSSettings['SKYWEB24_LOYAL_PROFILECOMPLETED_SMS']['refTemplate']) && $bonus>0){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_PROFILECOMPLETED_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_PROFILECOMPLETED_SMS']['refTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus),
	...
	private function getUserSMS($userId, $bonus, $percent){
		$SMSSettings=$this->profileSetting['sms_settings'];
		if(!empty($SMSSettings['SKYWEB24_LOYAL_PROFILECOMPLETED_SMS']['userTemplate']) && $bonus>0){
			$userData=Loyaltyprogram\Tools::getUserData($userId);
			if(!empty($userData['PERSONAL_PHONE'])){
				$tmpPhone=$userData['PERSONAL_PHONE'];
			}elseif(!empty($userData['PERSONAL_MOBILE'])){
			...
			if(!empty($tmpPhone)){
				...
				return [
					"EVENT_NAME" => "SKYWEB24_LOYAL_PROFILECOMPLETED_SMS",
					"MESSAGE_ID" => $SMSSettings['SKYWEB24_LOYAL_PROFILECOMPLETED_SMS']['userTemplate'],
					"LID" => $userData['LID'],
					"C_FIELDS" => [
						"USER_PHONE" => $tmpPhone,
						"BONUS"=>strval($bonus),
	...
	private function getUserFields(){
		...
		if(count($props)>0){
			$listProfile['prop']=[
				'title'=>Loc::getMessage("skyweb24.loyaltyprogram_PROFCOMPLETED_PROPS"),
				'child'=>$props
			];
		}
		return $listProfile;
	...
	private function drawProfile(){
		...
		<tr class="heading"><td colspan="2"><?=Loc::getMessage("skyweb24.loyaltyprogram_PROFCOMPLETED_TITLE")?></td></tr><?
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_PROFILECOMPLETED'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_PROFILECOMPLETED']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_PROFILECOMPLETED'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_PROFILECOMPLETED',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_EVENT_PROFILECOMPLETED"),
				'DESCRIPTION'=>'
					#EMAIL_TO# - '.Loc::getMessage("skyweb24.loyaltyprogram_PROFILECOMPLETED_EMAIL_TO").'
					#BONUS# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_PROFILECOMPLETED_BONUS").'
					#PERCENT# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_PROFILECOMPLETED_PERCENT").'
				',
				'SORT'=>500
			]
		];
	...
	protected function mailTemplates($key){
		...
		$mailTemplates = [
			'userTemplate'=>[
				...
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_PROFILECOMPLETED_USERBONUSADD"),
			...
			'refTemplate'=>[
				...
				'EMAIL_FROM'=>'#DEFAULT_EMAIL_FROM#',
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_PROFILECOMPLETED_REFBONUSADD"),
	...
	private function getUserEmail($userId, $bonus, $percent){
		...
		if(!empty($emailSettings['SKYWEB24_LOYAL_PROFILECOMPLETED']['userTemplate'])){
			$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_PROFILECOMPLETED']['userTemplate']);
			if($arEM = $rsEM->Fetch()){
				$sites=array_keys($this->globalSettings->getSites());
				...
				if(!empty($arUser['EMAIL'])){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_PROFILECOMPLETED",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_PROFILECOMPLETED']['userTemplate'],
	...
	private function getRefEmail($userId, $bonus, $percent){	
		$emailSettings=$this->profileSetting['email_settings'];
		if(!empty($emailSettings['SKYWEB24_LOYAL_PROFILECOMPLETED']['refTemplate'])){
			global $DB;
			$results=$DB->Query('select * from b_user where id='.$userId);
			$arUser = $results->Fetch();
			if(!empty($arUser['EMAIL'])){
				...
				if(!isset($this->refTemplate)){
					$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_PROFILECOMPLETED']['refTemplate']);
					if($arEM = $rsEM->Fetch()){
				...
				if($this->refTemplate!==false){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_PROFILECOMPLETED",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_PROFILECOMPLETED']['refTemplate'],
						"LANGUAGE_ID" => LANGUAGE_ID,
						"LID" => $sites[0],
						"C_FIELDS" => [
							"EMAIL_TO" => $arUser['EMAIL'],
	...
	*/
	...
	public function getConditions($mode=''){
		...
		$mainParams=[[
            'controlId'=> 'sites',
            'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_orderpay_sites"),
            'group'=> false,
            ...
            'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_orderpay_GroupRef"),
            'group'=> false,
            ...
            [
                ...
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_birthday_levelParentRef"),
                'group'=> false,
            ...
            [
                ...
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_compprofile_desc"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_compprofile"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_compprofile")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
                    ...
                    Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_UNIT_PERCENT")
                ]
            ],
            [
                'controlId'=> 'dateRegister',
                'description'=>Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_dateregister_desc"),
                'group'=> false,
                'label'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_dateregister"),
                'showIn'=> ['registerbyRef', 'registerbyRefSubGrp', 'registerbyParentRef', 'registerbyParentRefSubGrp'],
                'control'=> [[
                    'id'=> 'prefix',
                    'type'=> 'prefix',
                    'text'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LABEL_dateregister")
                ], [
                    'id'=> 'logic',
                    'name'=> 'logic',
                    'type'=> 'select',
                    'values'=> [
                        'more'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_LATER"),
                        'less'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_PREVIOUSLY")
                    ],
                    'defaultText'=> Loc::getMessage("skyweb24.loyaltyprogram_CONDITION_LOGIC_MORE"),
                    'defaultValue'=> 'more'
                ],[
						...
						if($status){
							if($nextO['controlId']=='registerbyRef'){
								...
								$insertArr=[
									...
									'status'=>'"inactive"',
									'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_PROFILECOMPLETED", ["#NUM#"=>$params['bonus']])).'"',
									'email'=>"'".$mailTmplt."'",
									'email'=>"'".$SMSTmplt."'"
								];
	...
	private function setReferalBonuses($nextUser, $nextO){
		...
		if((int) $params['bonus']>0 && count($newRew)>0){
			...
			foreach($newRew as $nextRew){
				...
				$insertArr['add_comment']='"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_REF_PROFILECOMPLETED", ["#NUM#"=>$params['bonus']])).'"';
