...
class BonusManager{
    ...
    public function bonus_payment($bonus=false,$user_id=false,$currency='RUB',$order_id=0,$debit='N'){
        ...
        if($debit=='Y'){
            ...
            \CSaleUserAccount::UpdateAccount(
                ...
                $bonus,
                $currency,
                //"skyweb24_bonus_payment".$order_id,
                "SKYWEB24_LOYAL_ORDERPAY",
                $order_id,
                Loc::getMessage("skyweb24.loyaltyprogram_PROGRAM_BONUSPAY", ["#NUM#"=>abs($bonus)])
            );
			$bonus = abs($bonus);
        }
        ...
        $transact=\CSaleUserTransact::GetList(['ID'=>'DESC'], [
			...
			'CURRENCY'=>$currency,
            'DEBIT'=>'N',
			//'DESCRIPTION'=>"skyweb24_bonus_payment".$order_id,
			'USER_ID'=>$user_id,
            'ORDER_ID'=>$order_id
        ]);
        $tran = $transact->fetch();
    ...
    public function bonus_ordering_refund($order_id=0,$user_id=0,$currency='RUB'){
        ...
        while($r=$res->fetch()){
            if($r['status']=='inactive'){
				...
				$comments[]=Loc::getMessage("skyweb24.loyaltyprogram_BONUS_OVERDUE_CANCEL", ["#NUM#"=>$r['bonus']]);
        ...
        if($bonus_refund>0){
            \CSaleUserAccount::UpdateAccount(
                ...
                '-'.$bonus_refund,
                $currency,
                "SKYWEB24_LOYAL_BONUSREFUND",
                $order_id,
                Loc::getMessage("skyweb24.loyaltyprogram_PROGRAM_BONUSREFUND_ORDERING", ["#NUM#"=>$bonus_refund])
            );
        }
    }
    ...
    public function bonus_refund($order_id=0,$user_id=0,$currency='RUB'){
        ...
        $transact=\CSaleUserTransact::GetList(['ID'=>'DESC'], [
			...
			'USER_ID'=>$user_id,
            'ORDER_ID'=>$order_id,
			'!DESCRIPTION'=>'SKYWEB24_LOYAL_BONUSREFUND'
		]);
        if($t=$transact->Fetch()){
            $amount = $t['AMOUNT'];
            ...
            if(!empty($bonuses) && count($bonuses)>0){
				...
				if($amount>0){
                        ...
                        \CSaleUserAccount::UpdateAccount(
                            ...
                            "SKYWEB24_LOYAL_BONUSREFUND",
                            $order_id,
                            Loc::getMessage("skyweb24.loyaltyprogram_PROGRAM_BONUSREFUND", ["#NUM#"=>round($amount,2)])
                        );
