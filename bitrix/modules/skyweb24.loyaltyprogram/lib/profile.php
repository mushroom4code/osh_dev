...
class Profile{
	...
	*/
	...
	public static function getProfileByType($type){
		$classNameNew='\Skyweb24\Loyaltyprogram\\'.$type;
	...
	public static function getProfileById($id){
		...
		$row['email_settings']=unserialize($row['email_settings']);
		$classNameNew='\\Skyweb24\\Loyaltyprogram\\'.$row['type'];
	...
	public function drawReferal(){
		...
		for($i=1; $i<=$maxLevel; $i++){
			...
			<tr class="ref_level"<?=$hide?>>
				<td width="40%"><?=Loc::getMessage("skyweb24.loyaltyprogram_PARAM_REWARDS", ["#NUM#"=>$i])?></td>
				<td>
		...
		if($additionalLink>0){?>
			<tr>
				<td width="40%"></td>
				<td>
					<a href="javascript:void(0);" class="ref_link"><?=Loc::getMessage("skyweb24.loyaltyprogram_PARAM_ADDREWARDS")?></a>
					<script>
	...
	protected function drawRow($type){
		switch ($type){
			...
			?>
			<tr><td width="40%"><?=Loc::getMessage("skyweb24.loyaltyprogram_PARAM_TYPE")?></td><td><?=$tmpProfiles[$this->profileSetting['type']]?></td></tr>
			...
			<tr><td width="40%"><?=Loc::getMessage("skyweb24.loyaltyprogram_PARAM_BONUS_SIZE_TURNOVER")?></td><td><input type="number" min="0" step="1" name="bonus_size_turnover" value="<?=$bonusSizeTurnover?>" /> <?=$options['currency']?> <?/*=Loc::getMessage("skyweb24.loyaltyprogram_PARAM_UNITS_BONUS")*/?></td></tr>
	...
	public function drawEmailList(){
		...
		foreach($this->profileSetting['email_settings'] as $keyEvent=>$nextEvent){
			...
			<tr>
				<td width="40%"><?=Loc::getMessage("skyweb24.loyaltyprogram_".$mailType['EVENT_NAME']."_".$keyTemplate)?></td>
				<td>
