...
class Profiles{
	...
	function __construct (){
		...
		$this->listProfiles['Registration']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_REGISTER");
		$this->listProfiles['Birthday']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_BIRTHDAY");
		$this->listProfiles['Ordering']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_BONUSADD");
		$this->listProfiles['Profilecompleted']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_PROFILECOMPLETED");
		if(\Bitrix\Main\Loader::includeModule('sender')){
			$this->listProfiles['Subscribe']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_SUBSCRIBE");
		}
		if(\Bitrix\Main\Loader::includeModule('blog') || \Bitrix\Main\Loader::includeModule('forum')){
			$this->listProfiles['Reviews']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_REVIEWS");
		}
        $this->listProfiles['Copyrighter']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_COPYRIGHTER");
		$this->listProfiles['Turnover']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_TURNOVER");
		$this->listProfiles['TurnoverRef']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_TURNOVER_REF");
		$this->listProfiles['Outersource']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_OUTERSOURCE");
		
		$this->listProfiles['separator']='separator';
		
		$this->listProfiles['Orderpay']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_BONUSPAY");
		$this->listProfiles['Writeoff']=Loc::getMessage("skyweb24.loyaltyprogram_PROFILE_WRITEOFF");
	}
	...
	public function initTableList(){
		...
		if($this->lAdmin->GroupAction() && $rights>'E'){
			...
			$tmpProfile=new \Skyweb24\Loyaltyprogram\Profiles\Profile;
			foreach($arID as $ID){
				if(strlen($ID)<=0)
					continue;
				$ID = IntVal($ID);
		...
		$this->lAdmin->AddHeaders(array(
			...
			array(
				"id" => "sort",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_TABLELIST_SORT"),
				"sort" => "sort",
				"default" => true
			),
			array(
				"id" => "active",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_TABLELIST_ACTIVE"),
				"sort" => "active",
				"default" => true
			  ),
			array(
				"id" => "name",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_TABLELIST_NAME"),
				"sort" => "name",
				"default" => true
			),
			array(
				"id" => "type",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_TABLELIST_TYPE"),
				"sort" => "type",
				"default" => false
			),
			array(
				"id" => "date_setting",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_TABLELIST_DATE_SETTING"),
				"sort" => "unixdata",
				"default" => true
			)
		));
		...
		while($arRes = $rsData->Fetch()){
			...
			$row->AddViewField("id", '<a href="./skyweb24_loyaltyprogram_profiles.php?lang='.SITE_ID.'&id='.$arRes['id'].'">'.$arRes['id'].'</a>');
			$tmpReady=($arRes['active']=='Y')?Loc::getMessage("skyweb24.loyaltyprogram_Y"):Loc::getMessage("skyweb24.loyaltyprogram_N");
			...
			$arActions[] = [
				...
				"ACTION"=>$this->lAdmin->ActionRedirect("./skyweb24_loyaltyprogram_profiles.php?id=".$arRes['id'])
			];
			$arActions[]=[
				"ICON"=>"copy",
				"TEXT"=>Loc::getMessage("skyweb24.loyaltyprogram_TABLE_COPY"),
				"ACTION"=>"if(confirm('".Loc::getMessage('skyweb24.loyaltyprogram_TABLE_COPY_CONFIRM')."')) ".$this->lAdmin->ActionRedirect("skyweb24_loyaltyprogram_profiles.php?action_button=copy&id=".$arRes['id'])
			];
			$arActions[] = [
				"ICON"=>"delete",
		...
		$this->lAdmin->AddFooter(
		 ...
		 if($rights > "E"){
			$this->lAdmin->AddGroupActionTable(Array(
				"edit"=>Loc::getMessage("skyweb24.loyaltyprogram_TABLE_EDIT"),
				"delete"=>Loc::getMessage("skyweb24.loyaltyprogram_TABLE_DELETE"),
				"activate"=>Loc::getMessage("skyweb24.loyaltyprogram_TABLE_ACTIVATE"),
				"deactivate"=>Loc::getMessage("skyweb24.loyaltyprogram_TABLE_DEACTIVATE")
			));
		 }
		$profilesMenu=[];
		foreach($this->listProfiles as $keyProf=>$nameProf){
			if($keyProf=='separator'){
				...
				$profilesMenu[]=[
					"TEXT"=>$nameProf,
					"ACTION"=>$this->lAdmin->ActionRedirect("./skyweb24_loyaltyprogram_profiles.php?id=new&type=".$keyProf)
				];
			}
		}
		...
		$aContext = array(
			array(
				"TEXT"=>Loc::getMessage("skyweb24.loyaltyprogram_ADD_PROFILE"),
				//"LINK"=>"javascript:parse('all');",
				"TITLE"=>Loc::getMessage("skyweb24.loyaltyprogram_ADD_PROFILE_TITLE"),
				"MENU"=>$profilesMenu,
			)
		);
