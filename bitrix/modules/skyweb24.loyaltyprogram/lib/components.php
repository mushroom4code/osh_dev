...
class Components{
	...
	public function getOverdueTransactions(){
		global $DB;
		$overdue=[];
		$res=$DB->Query('select id, bonus_start,bonus,user_id,user_bonus,order_id,currency,profile_type,profile_id,status,'.$DB->DateToCharFunction("date_add").' date_add,'.$DB->DateToCharFunction("date_remove").'date_remove,add_comment,comments,email from '.$this->globalSettings->getTableBonusList().' where user_id='.$this->idUser.' and status="active" and date_remove>now() order by id desc');
		if($row = $res->Fetch()){
	...
	public function getUserTransactions($arNavParams, $arrFilter=[], $orders=['id'=>'desc']){
		...
		$filterStr='(skyweb24_loyal_bonuses.user_id='.$this->idUser.' or skyweb24_loyal_bonuses.user_id is null) and b_sale_user_transact.USER_ID='.$this->idUser.' and b_sale_user_transact.CURRENCY="'.$this->moduleOptions['currency'].'"';
		...
		$dbTransactions=$DB->Query('select
			...
			group_concat(skyweb24_loyal_bonuses.order_id) as ORDER_ID2,
			...
			group_concat(skyweb24_loyal_bonuses.bonus_start) as bonus_start,
			group_concat('.$DB->DateToCharFunction("skyweb24_loyal_bonuses.date_add").') as date_add,
			group_concat('.$DB->DateToCharFunction("skyweb24_loyal_bonuses.date_remove").') as date_remove,
			group_concat(skyweb24_loyal_bonuses.add_comment) as add_comment

			from b_sale_user_transact
			left join skyweb24_loyal_bonuses_transaction
			on(skyweb24_loyal_bonuses_transaction.transaction_id=b_sale_user_transact.ID and b_sale_user_transact.DEBIT="Y")
			left join skyweb24_loyal_bonuses
			on (skyweb24_loyal_bonuses_transaction.bonus_id=skyweb24_loyal_bonuses.id)
	...
	private function checkEvent(){
		$res=\Bitrix\Main\Mail\Internal\EventTypeTable::getList(
			['filter'=>['EVENT_NAME'=>'SKYWEB24_LOYAL_SHARE']]
		);
		if(!$nextRes=$res->fetch()){
			$fieldsEvent=[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_SHARE',
				'NAME'=>Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SHARE"),
				'DESCRIPTION'=>'
					#EMAIL_TO# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SHARE_EMAIL_TO").'
					#REF_LINK# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SHARE_REF_LINK").'
					#COUPON# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SHARE_COUPON").'
					#NAME# - '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SHARE_NAME").'
				',
				'SORT'=>500
			];
	...
	private function shareCoupon($request){
		...
		if($restoreTemplate){
			...
			$templateId=\Bitrix\Main\Mail\Internal\EventMessageTable::add([
				...
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SHARE_TMPLT_COUPON"),
	...
	private function shareEmail($request){
		...
		if($restoreTemplate){
			...
			$templateId=\Bitrix\Main\Mail\Internal\EventMessageTable::add([
				...
				'EMAIL_FROM'=>'#DEFAULT_EMAIL_FROM#',
				'EMAIL_TO'=>'#EMAIL_TO#',
				'SUBJECT'=>'#SITE_NAME#: '.Loc::getMessage("skyweb24.loyaltyprogram_EVENT_SHARE_TMPLT_REF_LINK"),
	...
	public function setWriteOffBonus($bonus, $regId){
		if(empty((int) $regId)){
			return false;
		}
