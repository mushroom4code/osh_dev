...
class Referrals{
	...
	*/
    ...
    public function setReferral2($parenRef, $ref, $type='manual'){
        global $DB;
        $this->type=$type;
        if(
            (int) $ref>0 &&
            ...
            return '/bitrix/admin/skyweb24_loyaltyprogram_referrals.php?lang='.LANGUAGE_ID.'&set_filter=Y&find_id='.$parenRef.'&by=date_create&order=desc';
        }
        return false;
    }
	...
	public function setReferralForm(){
        ...
        <form action="<?=$APPLICATION->GetCurPageParam("", array("id"));?>" method="post" name="get_user_id" class="newRefLigament">
            <h4><?=Loc::getMessage("skyweb24.loyaltyprogram_CREATE_NEW_REF")?></h4>
            <div>
                <span class="title"><?=Loc::getMessage("skyweb24.loyaltyprogram_HEADER_REF")?>: </span>
			...
			<div>
                <span class="title"><?=Loc::getMessage("skyweb24.loyaltyprogram_LINK_REF")?>: </span>
			...
			<?ShowError(Loc::getMessage("skyweb24.loyaltyprogram_EMPTY_REF_NOTE"));?>
			<?/*?><div><?=ShowNote(Loc::getMessage("skyweb24.loyaltyprogram_NOTUSECHANGE"));?></div><?*/?>
            <div>
                <input class="tablebodybutton" disabled="disabled" type="button" name="create_ref" id="create_ref" value="<?=Loc::getMessage("skyweb24.loyaltyprogram_CREATE_REF")?>">
            </div>
        </form>
    <script>
    ...
    BX.bind(BX('create_ref'), 'click', function(e) {
        ...
        _this.disabled=true;
        BX.ajax({
            url: '/bitrix/admin/skyweb24_loyaltyprogram_referrals.php?lang=<?=LANGUAGE_ID?>',
            data: {
                'ajax':'y',
                'setRef':'y',
    ...
    function getUserId(){
        ...
        if (!!nameUser){
            if (
                ...
                cUseId=document.get_user_id['user_id_input'].value;
                if (cUseId!=''){
                    nameUser.innerHTML = '<i><?=Loc::getMessage("skyweb24.loyaltyprogram_SEARCH")?>...</i>';
					if(BX('user_id_input').value==BX('linkuser_id_input').value){
						setTimeout(function(){
                            nameUser.innerHTML='<span class="error"><?=Loc::getMessage("skyweb24.loyaltyprogram_USERMATCH")?></span>';
							activeSetReferral();
						},1000);
					}
                    else if (cUseId!=1)
                    {
                        ...
                        if(statusUser=='notExists'){
                            setTimeout(function(){
                                nameUser.innerHTML='<span class="error"><?=Loc::getMessage("skyweb24.loyaltyprogram_NOTUSEREXIST")?></span>';
                            },1000);
                        }
                        setTimeout(function(){
                            activeSetReferral();
                    ...
                    {
                        nameUser.innerHTML = '[<a title=\"<?=Loc::getMessage("skyweb24.loyaltyprogram_HEADER_USER_ID")?>\" class=\"tablebodylink\" target=\"_blank\" href=\"/bitrix/admin/user_edit.php?ID=<?=$USER->GetID()?>&lang=<?=LANGUAGE_ID?>\"><?=$USER->GetID()?><\/a>] <?=$USER->GetFullName()?>';
                        statusUser=loyaltyTools.checkUser(cUseId);
                        setTimeout(function(){
                            activeSetReferral();
                        },1000);
        ...
        if (!!linkUser){
            if (
                ...
                cLinkUseId=document.get_user_id['linkuser_id_input'].value;
                if (cLinkUseId!=''){
                    linkUser.innerHTML = '<i><?=Loc::getMessage("skyweb24.loyaltyprogram_SEARCH")?>...</i>';

                    if(BX('user_id_input').value==BX('linkuser_id_input').value){
						setTimeout(function(){
                            linkUser.innerHTML='<span class="error"><?=Loc::getMessage("skyweb24.loyaltyprogram_USERMATCH")?></span>';
							activeSetReferral();
						},1000);
					}
                    else if (cLinkUseId!=1)
                    {
                        ...
                        if(statusLinkUser=='notExists'){
                            setTimeout(function(){
                                linkUser.innerHTML='<span class="error"><?=Loc::getMessage("skyweb24.loyaltyprogram_NOTUSEREXIST")?></span>';
                            },1000);
                        }else if(statusLinkUser=='referral'){
                            setTimeout(function(){
                                linkUser.innerHTML=linkUser.innerHTML+' <span class="error"><?=Loc::getMessage("skyweb24.loyaltyprogram_USERISREFERRAL")?></span>';
                            },1000);
                        }
                        setTimeout(function(){
                            activeSetReferral();
                    ...
                    {
                        linkUser.innerHTML = '[<a title=\"<?=Loc::getMessage("skyweb24.loyaltyprogram_HEADER_USER_ID")?>\" class=\"tablebodylink\" target=\"_blank\" href=\"/bitrix/admin/user_edit.php?ID=<?=$USER->GetID()?>&lang=<?=LANGUAGE_ID?>\"><?=$USER->GetID()?><\/a>] <?=$USER->GetFullName()?>';
                        statusLinkUser=loyaltyTools.checkUser(cLinkUseId, 'notreferral');
                        setTimeout(function(){
                            activeSetReferral();
                        },1000);
	...
	public function initFilter(){
		...
		$oFilter = new \CAdminFilter(
			...
			[
				Loc::getMessage("skyweb24.loyaltyprogram_HEADER_REF"),
				Loc::getMessage("skyweb24.loyaltyprogram_HEADER_USERS"),
				Loc::getMessage("skyweb24.loyaltyprogram_HEADER_DATE_CREATE"),
				Loc::getMessage("skyweb24.loyaltyprogram_HEADER_LEVEL"),
				Loc::getMessage("skyweb24.loyaltyprogram_HEADER_TYPE"),
				Loc::getMessage("skyweb24.loyaltyprogram_HEADER_SOURCE")
			]
		);?>
		<form name="find_form" method="get" action="<?echo $APPLICATION->GetCurPage();?>">
		<?$oFilter->Begin();?>
		<tr>
			<td><b><?=Loc::getMessage("skyweb24.loyaltyprogram_HEADER_REF")?>:</b></td>
			<td>
                <?echo \FindUserID("find_id", $find_id, "", "find_form");?>
			</td>
		</tr>
        <tr>
			<td><b><?=Loc::getMessage("skyweb24.loyaltyprogram_HEADER_USERS")?>:</b></td>
			<td>
                <?echo \FindUserID("find_user", $find_user, "", "find_form");?>
			</td>
		</tr>
        <tr>
			<td><b><?=Loc::getMessage("skyweb24.loyaltyprogram_HEADER_DATE_CREATE")?>:</b></td>
			<td>
                <?echo CalendarPeriod("find_date_from", $find_date_from, "find_date_to", $find_date_to, "find_form", "Y")?>
			</td>
		</tr>
		<tr>
			<td><?=Loc::getMessage("skyweb24.loyaltyprogram_HEADER_LEVEL").":"?></td>
			<td>
                <select name="find_level">
                    <option value="">...</option>
                    <?
                        $rsData = $DB->Query('select max(level) as level from skyweb24_loyal_users');
                        $maxLevel=0;
                        if($row=$rsData->Fetch()){
                            $maxLevel=$row['level'];
                        }
        ...
        <tr>
			<td><?=Loc::getMessage("skyweb24.loyaltyprogram_HEADER_TYPE").":"?></td>
			<td>
            ...
            <tr>
			<td><?=Loc::getMessage("skyweb24.loyaltyprogram_HEADER_SOURCE").":"?></td>
			<td>
    ...
    private function getByResource($resource, $mode='table'){
        global $DB;
        $users = [];
        $rsData = $DB->Query('select * from skyweb24_loyal_coupons where coupon="' . $resource . '";');
        if($row = $rsData->Fetch()){
            $type='coupon';
            $source_link=$row['id'];
        }else{
            ...
            $rsData = $DB->Query('select * from skyweb24_loyal_partner_sites where site like "%' . $resource . '%";');
            if($row = $rsData->Fetch()){
                $type='partnerSite';
                $source_link=$row['id'];
            }
	...
	public function initTableList(){
        ...
        from '.$this->settings->getTableUsersList().' left join skyweb24_loyal_users as ref
		...
		$this->lAdmin->NavText($rsData->GetNavPrint(Loc::getMessage("skyweb24.loyaltyprogram_TABLELIST_PAGINATOR")));
		$this->lAdmin->AddHeaders(array(
			...
			array(
				"id" => "level",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_HEADER_LEVEL"),
				"sort" => "level",
				"default" => true
			),
			array(
				"id" => "ref",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_HEADER_REF"),
				"sort" => "ref_login",
				"default" => true
			),
			array(
				"id" => "user",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_HEADER_USERS"),
				"sort" => "user",
				"default" => true
			),
			array(
				"id" => "date_create",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_HEADER_DATE_CREATE"),
				"sort" => "date_create_sort",
				"default" => true
			),
			array(
				"id" => "type",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_HEADER_TYPE"),
				"sort" => "type",
				"default" => true
			),
			array(
				"id" => "source",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_HEADER_SOURCE"),
				"sort" => "source",
				"default" => true
			)
		));
		...
		while($arRes = $rsData->Fetch()){
			...
			$arActions[] = [
				"ICON"=>"delete",
				"TEXT"=>Loc::getMessage("skyweb24.loyaltyprogram_TABLE_DELETE"),
				"ACTION"=>"if(confirm('".Loc::getMessage('skyweb24.loyaltyprogram_TABLE_DELETE_CONFIRM')."')) ".$this->lAdmin->ActionDoGroup($arRes['id'], "delete")
			];
		...
		$aContext = [
			[
				"TEXT"=>Loc::getMessage("skyweb24.loyaltyprogram_REF_TREE_MODE"),
				"LINK"=>"skyweb24_loyaltyprogram_referrals.php?mode=tree&lang=".LANG,
				"TITLE"=>Loc::getMessage("skyweb24.loyaltyprogram_REF_TREE_MODE")
			]
		];
		...
		if($rights > "E"){
			$this->lAdmin->AddGroupActionTable([
				"delete"=>Loc::getMessage("skyweb24.loyaltyprogram_TABLE_DELETE")
			]);
		 }
