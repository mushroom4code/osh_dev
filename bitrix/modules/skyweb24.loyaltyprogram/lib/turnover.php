...
class Turnover extends Profile implements Profileinterface{
	...
	protected function checkEmailList(){
		...
		if(empty($this->profileSetting['email_settings']['SKYWEB24_LOYAL_BONUSTURNOVER'])){
			$this->profileSetting['email_settings']['SKYWEB24_LOYAL_BONUSTURNOVER']=[
				'userTemplate'=>0,
				'refTemplate'=>0
			];
		}
	...
	protected function mailType($type){
		$mailType = [
			'SKYWEB24_LOYAL_BONUSTURNOVER'=>[
				'LID'=>LANGUAGE_ID,
				'EVENT_NAME'=>'SKYWEB24_LOYAL_BONUSTURNOVER',
	...
	public function setBonus($event=false){
		if($this->profileSetting['settings']['bonuses']['bonus_add']>0 && !empty($this->profileSetting['settings']['bonuses']['bonus_period']) && $this->ruleCheck()==true){
			...
			//already turnover
			$alreadyTurnover=[];
			$res=$DB->Query('select * from skyweb24_loyal_bonuses where date_add=FROM_UNIXTIME('.$timePeriod['endTime'].') and profile_id='.$this->profileSetting['id'].';');
			while($row = $res->Fetch()){
			...
			while($row = $res->Fetch()){
				if(!in_array($row['user_id'], $alreadyTurnover)){
					...
					$idIns=$DB->Insert($this->globalSettings->getTableBonusList(), [
						...
						'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_BONUSTURNOVER_ADD", ["#NUM#"=>$bonusAdd])).'"',
						'email'=>"'".$mailTmplt."'"
					], $err_mess.__LINE__);
	...
	private function getUserEmail($userId, $bonus, $turnover){
		...
		if(!empty($emailSettings['SKYWEB24_LOYAL_BONUSTURNOVER']['userTemplate'])){
			$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_BONUSTURNOVER']['userTemplate']);
			if($arEM = $rsEM->Fetch()){
				$sites=array_keys($this->globalSettings->getSites());
				...
				if(!empty($arUser['EMAIL'])){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_BONUSTURNOVER",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_BONUSTURNOVER']['userTemplate'],
	...
	private function setReferalBonuses($userId, $price, $turnover){
		global $DB;
		$timePeriod=$this->getInterval();
		$res=$DB->Query('select * from skyweb24_loyal_bonuses where date_add=FROM_UNIXTIME('.$timePeriod['endTime'].') and profile_id='.$this->profileSetting['id'].' and user_bonus='.$userId.';');
		if(!$row = $res->Fetch()){
			...
			foreach($rewards as $key=>$val){
				if($val>0){
					...
					$idIns=$DB->Insert($this->globalSettings->getTableBonusList(), [
						...
						'date_add'=>'FROM_UNIXTIME('.$startBonus.')',
						'date_remove'=>$endBonus,
						'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_BONUSTURNOVERREF_ADD", ["#NUM#"=>$val])).'"',
						'email'=>"'".$mailTmplt."'"
					], $err_mess.__LINE__);
					Eventmanager::manageBonuses($idIns);
				}
	...
	private function getRefEmail($userId, $bonus, $turnover){
		$emailSettings=$this->profileSetting['email_settings'];
		if(!empty($emailSettings['SKYWEB24_LOYAL_BONUSTURNOVER']['refTemplate'])){
			...
			if(!empty($arUser['EMAIL'])){
				...
				if(!isset($this->refTemplate)){
					$rsEM = \CEventMessage::GetByID($emailSettings['SKYWEB24_LOYAL_BONUSTURNOVER']['refTemplate']);
					if($arEM = $rsEM->Fetch()){
				...
				if($this->refTemplate!==false){
					return [
						"EVENT_NAME" => "SKYWEB24_LOYAL_BONUSTURNOVER",
						"MESSAGE_ID" => $emailSettings['SKYWEB24_LOYAL_BONUSTURNOVER']['refTemplate'],
						"LANGUAGE_ID" => LANGUAGE_ID,
						"LID" => $sites[0],
						"C_FIELDS" => [
							"EMAIL_TO" => $arUser['EMAIL'],
