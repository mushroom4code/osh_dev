	...
	*/
	public function setBonus($userId=''){
		if($this->profileSetting['settings']['bonuses']['bonus_size']>0 && $this->ruleCheck()==true){
			...
			if(count($tmpUsers)>0){
				$res = Application::getConnection()->query('select user_id, max(date_add) as date_add, CURDATE() from skyweb24_loyal_bonuses where profile_type="'.$this->profileSetting['type'].'" and date_add >= CURDATE() group by user_id;');
			...
			if(count($tmpUsers)>0){
				...
				foreach($tmpUsers as $nextUser){
					...
					$connection->queryExecute("insert into ".$this->globalSettings->getTableBonusList()." (bonus_start, bonus, user_id, currency, profile_type, profile_id, status, date_add, date_remove, add_comment, email) values(
						...
						".$endBonus.",
						'".$sqlHelper->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_USER_BIRTHDAY", ["#NUM#"=>$this->profileSetting['settings']['bonuses']['bonus_size']]))."',
						'".$mailTmplt."'
					);");
		...
		foreach($rewards as $key=>$val){
			if($val>0){
				...
				$idIns=$DB->Insert($this->globalSettings->getTableBonusList(), [
					...
					'add_comment'=>'"'.$DB->ForSql(Loc::getMessage("skyweb24.loyaltyprogram_BONUS_REF_BIRTHDAY", ["#NUM#"=>$val])).'"',
					'email'=>"'".$mailTmplt."'"
				], $err_mess.__LINE__);
