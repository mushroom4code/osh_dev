...
class Queue{
	...
	public static function deleteFromQueue($id){
        ...
        if($arData = $data->fetch()) {
            Entity\BonusesTable::update($id, [
                'status'=>'overdue',
                'comments'=>self::getNewComment($arData, Loc::getMessage("skyweb24.loyaltyprogram_TABLE_BONUS_DELETE_MANUAL").' '.date('d.m.Y H:i'))
            ]);
            return true;
        }
        return false;
		...
		if(!empty($_REQUEST['FIELDS_OLD'][$id])){
			...
			if(count($updateArr)>0){
				...
				if($arRes = $rsData->Fetch()){
					...
					$DBUpdateArr['comments']='"'.$DB->ForSql(self::getNewComment($arRes, Loc::getMessage("skyweb24.loyaltyprogram_TABLE_BONUS_UPDATE_MANUAL").' '.date('d.m.Y H:i'))).'"';
	...
	public function initTableList(){
		...
		$this->lAdmin->AddHeaders(array(
			...
			["id" => "bonus","content" => Loc::getMessage("skyweb24.loyaltyprogram_QUEUE_HEADER_bonus"), "sort" => "bonus","default" => true],
			["id" => "user_id","content" => Loc::getMessage("skyweb24.loyaltyprogram_QUEUE_HEADER_user_id"),"sort" => "user_id","default" => true],
			["id" => "user_bonus","content" => Loc::getMessage("skyweb24.loyaltyprogram_QUEUE_HEADER_user_bonus"),"sort" => "user_bonus","default" => true],
			["id" => "order_id","content" => Loc::getMessage("skyweb24.loyaltyprogram_QUEUE_HEADER_order_id"),"sort" => "order_id","default" => true],
			["id" => "profile_type","content" => Loc::getMessage("skyweb24.loyaltyprogram_QUEUE_HEADER_profile_type"),"sort" => "profile_type","default" => true],
			["id" => "profile_id","content" => Loc::getMessage("skyweb24.loyaltyprogram_QUEUE_HEADER_profile_id"),"sort" => "profile_id","default" => true],
			["id" => "date_add","content" => Loc::getMessage("skyweb24.loyaltyprogram_QUEUE_HEADER_date_add"),"sort" => "date_add","default" => true],
			["id" => "date_remove","content" => Loc::getMessage("skyweb24.loyaltyprogram_QUEUE_HEADER_date_remove"),"sort" => "date_remove","default" => true]
		));
		while($arRes = $rsData->Fetch()){
            ...
            $row->AddViewField("user_bonus", $arRes['user_bonus']>0?Loc::getMessage("skyweb24.loyaltyprogram_Y"):Loc::getMessage("skyweb24.loyaltyprogram_N"));

			$arActions = [];
			$arActions[] = [
				"ICON"=>"delete",
				"TEXT"=>Loc::getMessage("skyweb24.loyaltyprogram_TABLE_DELETE"),
				"ACTION"=>"if(confirm('".Loc::getMessage('skyweb24.loyaltyprogram_TABLE_DELETE_BONUSQUEUE')."')) ".$this->lAdmin->ActionDoGroup($arRes['ID'], "delete")
			];
			$arActions[] = [
				"ICON"=>"edit",
				"TEXT"=>Loc::getMessage("skyweb24.loyaltyprogram_TABLE_EDIT"),
				"ACTION"=>"if(confirm('".Loc::getMessage('skyweb24.loyaltyprogram_TABLE_EDIT_BONUSQUEUE')."')) ".$this->lAdmin->ActionDoGroup($arRes['ID'], "edit")
			];
