...
//list part
	public function initFilter(){
		...
		$oFilter = new \CAdminFilter(
			...
			[
				Loc::getMessage("skyweb24.loyaltyprogram_WO_FILTER_USER"),
				Loc::getMessage("skyweb24.loyaltyprogram_WO_FILTER_STATUS")
			]
		);?>
		<form name="find_form" method="get" action="<?echo $APPLICATION->GetCurPage();?>">
		<?$oFilter->Begin();?>
		<tr>
			<td><b><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_FILTER_USER")?>:</b></td>
			<td>
				<input type="text" min="1" step="1" size="25" name="find_user" value="<?echo htmlspecialchars($find_user)?>">
			</td>
		</tr>
		<tr>
			<td><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_FILTER_STATUS").":"?></td>
			<td>
	...
	public function initTableList(){
		...
		$this->lAdmin->AddHeaders(array(
			...
			array(
				"id" => "bonus",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_WO_HEADER_BONUS"),
				"sort" => "bonus",
				"default" => true
			),
			array(
				"id" => "status",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_WO_HEADER_STATUS"),
				"sort" => "status",
				"default" => true
			),
			array(
				"id" => "comment",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_WO_HEADER_COMMENT"),
				"sort" => "comment",
				"default" => false
			),
			array(
				"id" => "user",
				"content" => Loc::getMessage("skyweb24.loyaltyprogram_WO_HEADER_USER"),
				"sort" => "last_name",
				"default" => true
			)
		));
		...
		while($arRes = $rsData->Fetch()){
			...
			$row->AddViewField("id", '<a href="/bitrix/admin/skyweb24_loyaltyprogram_writeoff.php?id='.$arRes['id'].'&lang='.SITE_ID.'">'.$arRes['id'].'</a>');
            ...
            if($arRes['status']=='request'){
                $arActions[] = [
                    "ICON" => "edit",
                    "DEFAULT" => true,
                    "TEXT" => Loc::getMessage("skyweb24.loyaltyprogram_TABLE_EDIT"),
                    "ACTION" => $this->lAdmin->ActionRedirect('/bitrix/admin/skyweb24_loyaltyprogram_writeoff.php?id=' . $arRes['id'] . '&lang=' . LANGUAGE_ID)
                ];
            }else{
                $arActions[] = [
                    "ICON" => "view",
                    "DEFAULT" => true,
                    "TEXT" => Loc::getMessage("skyweb24.loyaltyprogram_TABLE_VIEW"),
                    "ACTION" => $this->lAdmin->ActionRedirect('/bitrix/admin/skyweb24_loyaltyprogram_writeoff.php?id=' . $arRes['id'] . '&lang=' . LANGUAGE_ID)
                ];
            }
...
//edit part
	...
	private function clearBonus(){
		...
		foreach($sqls as $sql){
			...
			while($row=$rsData->Fetch()){
				...
				$comments[]=Loc::getMessage("skyweb24.loyaltyprogram_BONUS_WRITE_OFF", ["#NUM#"=>$writeBonus]);
		...
		if($this->isEdit()){
			if($data['status']=='execute'){
				\CSaleUserTransact::Update(
					$this->dataOrder['transact_id'],
					['NOTES'=>Loc::getMessage("skyweb24.loyaltyprogram_PROGRAM_WRITEOFF_EXECUTE")]
				);
				...
				\CSaleUserAccount::UpdateAccount(
					...
					'',
					Loc::getMessage("skyweb24.loyaltyprogram_PROGRAM_WRITEOFF_REJECT")
				);
                $profile=Profiles\Profile::getProfileById($this->dataOrder['profile_id']);
                $profile->sendEvent([
	...
	public function getEditArea(){?>
		<tr><td width="40%"><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_NAME")?></td><td><a href="/bitrix/admin/user_edit.php?lang=<?=SITE_ID?>&ID=<?=$this->dataOrder['user_id']?>" target="_blank"><?=$this->dataOrder['fill_name'].' ['.$this->dataOrder['user_id'].']'?></a></td></tr>
		<tr><td><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_BONUS")?></td><td><?=\CCurrencyLang::CurrencyFormat($this->dataOrder['bonus'], $this->moduleOptions['currency'])?></td></tr>
		<tr><td><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_DATE_ORDER")?></td><td><?=$this->dataOrder['date_order']?></td></tr>
		<?if(!empty($this->dataOrder['cart_number'])){?>
		<tr><td><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_CART_NUMBER")?></td><td><?=$this->dataOrder['cart_number']?></td></tr>
		<?}else{?>
		<tr><td><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_INVOICE_NUMBER")?></td><td><?=$this->dataOrder['invoice']?></td></tr>
		<tr><td><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_BIK_NUMBER")?></td><td><?=$this->dataOrder['bik']?></td></tr>
		<?}?>
		<?if($this->isEdit()){?>
			<tr><td><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_SET_STATUS")?></td><td><select name="status"><?foreach($this->statuses as $statusKey=>$statusName){
			...
			<tr><td><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_COMMENT")?></td><td><textarea name="comment" id="" cols="50" rows="5"></textarea></td></tr>
		<?}else{?>
			<tr><td><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_FILTER_STATUS")?></td><td><?=$this->statuses[$this->dataOrder['status']]?></td></tr>
		<?}?>
	<?}
	...
	private function setLog($data){
		...
		$cLog[]=['status'=>$data['status'], 'comment'=>$data['comment'], 'date'=>date($DB->DateFormatToPHP(\CSite::GetDateFormat("FULL")), time()), 'manader_id'=>$USER->GetID()];
	...
	public function getLog(){?>
		<tr><td width="40%"><?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_DATE_ORDER")?></td><td><?=$this->dataOrder['date_order']?></td></tr>
		<?if(!empty($this->dataOrder['log']) && is_array($this->dataOrder['log'])){
			foreach($this->dataOrder['log'] as $nextRow){
				...
				?>
				<tr><td><?= $this->statuses[$this->dataOrder['status']]?></td>
				<td>
					<?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_TIME")?>: <?=$nextRow['date']?><br>
					<?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_RESPONSIVE")?>: <a href="/bitrix/admin/user_edit.php?lang=<?=SITE_ID?>&ID=<?=$nextRow['manader_id']?>" target="_blank"><?=$user['FULL_NAME'].' ['.$nextRow['manader_id'].']'?></a><br>
					<?=Loc::getMessage("skyweb24.loyaltyprogram_WO_DET_COMMENT")?>: <br><?=$nextRow['comment']?>
				</td></tr>
			<?}
		}?>
	<?}
