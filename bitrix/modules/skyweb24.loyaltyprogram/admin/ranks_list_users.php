...
$ranks=new \Skyweb24\Loyaltyprogram\RanksUsers;

if(!empty($_REQUEST['id']) && ($_REQUEST['id']=='new' || (int) $_REQUEST['id']>0)){
	if($_REQUEST['id']=='new'){
		$APPLICATION->SetTitle(Loc::getMessage("skyweb24.loyaltyprogram_TITLE_RANK_NEW"));	
	}else{
		$APPLICATION->SetTitle(Loc::getMessage("skyweb24.loyaltyprogram_TITLE_RANK_EDIT"));	
	}
}else{
	...
	if($RankId == 0 || $request['id'] !='new'){
	    if($res->isSuccess()) {
            ...
            $info = new CAdminMessage([
                "TYPE" => "OK",
                "MESSAGE" => Loc::getMessage("skyweb24.ranks_SAVE_SETTING_SUCCESS")
            ]);
        }else{
            $info= new CAdminMessage([
                "TYPE"=>"ERROR",
		...
		$info= new CAdminMessage([
			"TYPE"=>"ERROR",
			"MESSAGE"=> Loc::getMessage("skyweb24.ranks_list_SAVE_SETTING_ERROR", ["#LINK#" => $APPLICATION->GetCurPage().'?lang='.LANGUAGE_ID.'&id='.$id]),
			"HTML" => true
		]);
	}
}elseif(!empty($request['ajax']) && $request['ajax']=='Y' && !empty($request['command'])){
	if($request['command']=='update_ranks'){
		\Skyweb24\Loyaltyprogram\Ranks::setRanks();
...
if(!empty($request['add']) && $request['add']=='success' && !empty($request['id'])){
	$info=new CAdminMessage([
		"TYPE"=>"OK",
		"MESSAGE"=>Loc::getMessage("skyweb24.ranks_ADD_RANK_SUCCESS")
	]);
}
...
?>
 <form name="bidding_setting" method="post" action="<?echo $APPLICATION->GetCurPage()?>?id=<?=$_REQUEST['id']?>&lang=<?=LANGUAGE_ID?>">
	...
	if($_REQUEST['id'] == 'new'){
		$aTabs = [
			["DIV" => "sw24_loyal_rank_edit", "TAB" => Loc::getMessage("skyweb24.ranks_list_new_TITLE"), "TITLE" => Loc::getMessage("skyweb24.ranks_list_new_TAB_RANK_TITLE")]
		];
	}else{
		$aTabs = [
			["DIV" => "sw24_loyal_rank_edit", "TAB" => Loc::getMessage("skyweb24.ranks_list_edit_TITLE"), "TITLE" => Loc::getMessage("skyweb24.ranks_list_edit_TAB_RANK_TITLE")]
		];
	}
	...
	?>
	<tr>
		<td style="text-align: right" width="50%"><?= Loc::getMessage("skyweb24.ranks_edit_ACTIVE")?></td>
		<td style="text-align: left" width="50%"><input type="checkbox" name="active" value="Y"<?=$data['active']=='Y'?' checked':'';?>></td>
	</tr>
	<tr>
		<td style="text-align: right" width="50%"><?=Loc::getMessage("skyweb24.ranks_list_edit_USER")?></td>
		<?if($_REQUEST['id'] == 'new'){?>
			<td>
				<input type="text" name="user_id" id="user_id_input" value="<?= $cUserId ?>" size="3" maxlength=""class="typeinput">
		...
		<?}else{?>
			<td style="text-align: left" width="50%"><input type="hidden" name="user_id" value="<?=$data['user_id']?>"><a target="_blank" href="/bitrix/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$arUser["ID"]?>">[<?=$arUser["ID"]?>]</a><?=$arUser["NAME"]?> <?=$arUser["LAST_NAME"]?></td>
		<?}?>
	</tr>
	<tr>
		<td style="text-align: right" width="50%"><?= Loc::getMessage("skyweb24.ranks_edit_NAME")?></td>
