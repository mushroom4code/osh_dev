...
function displaySelectProp($module_id, $arSite, $optionName, $default,$arList)
{
    $nameUpper = strtoupper($optionName);
    $nameLower = strtolower($optionName);
    ?>
    <tr>
        <td align="right" width="300"><label for="<?= $nameUpper . '_' . $arSite['ID'] ?>"><?= GetMessage('OPT_' . $nameUpper) ?></label></td>
        <td>
	          <? $value = COption::GetOptionString($module_id, $nameLower, $default, $arSite['ID']); ?>
            <select name="<?= $nameUpper . '_' . $arSite['ID'] ?>" >
	             <?foreach($arList as $id=>$name):?>
		             <option <?= ($id==$value)? 'selected': ''?> value="<?=$id?>"><?=$name?></option>
							 <? endforeach ?>
            </select>
        </td>
    </tr>
    ...
    if (ToUpper($REQUEST_METHOD) == 'POST' &&
        ...
        if (strlen($RestoreDefaults) > 0) {
            ...
            foreach ($arSites as $arSite) {
                ...
                setSimpleOption($module_id, $arSite, 'PROVIDE_LINK_BY');
                ...
                foreach ($arSites as $arSite) {
                    ...
                    <table cellspacing="5" cellpadding="0" border="0" width="100%" align="center">
                                            ...
                                            <? } else { ?>
                                                ...
                                                <select name="<?= $fieldName ?>" id="<?= $fieldName ?>">
                                                    ...
                                                    <? foreach (CDadataSuggestionsSettings::GetSuggestionsFields() as $arSugGroup => $arSugNames): ?>
                                                        <optgroup label="<?= GetMessage('GORILLAS_SUGGESTIONS_GROUP_' . $arSugGroup . '_NAME') ?>">
                                                            <? if (!empty($arSugNames)): ?>
                        ...
                        ?>
	                      <?
	                      $arList=array(
		                      'id' =>GetMessage("SALE_SLS_PROVIDE_LINK_BY_PARAMETER_ID"),
		                      'code' => GetMessage("SALE_SLS_PROVIDE_LINK_BY_PARAMETER_CODE")
	                      );
	                      displaySelectProp($module_id, $arSite, "PROVIDE_LINK_BY", "code", $arList); ?>
