...
class DellindeliveryApicore
{
    ...
    protected static $apiUrls = array(
        'authorize' => 'http://api.dellin.stage/v1/customers/login.json',
        'calculator' => 'https://api.dellin.ru/v2/calculator.json',
        'citySearch' => 'https://www.dellin.ru/api/cities/search.json',
        'login' => 'http://api.dellin.stage/v1/customers/login.json',
        'request' => 'http://api.dellin.stage/v2/request',
        'tracker' => 'http://api.dellin.stage/v2/public/tracker.json',
        'produceDate' => 'http://api.dellin.stage/v1/public/produce_date.json',
        'requestTerminal' => 'http://api.dellin.stage/v1/public/request_terminals.json',
        'terminals' => 'http://api.dellin.stage/v3/public/terminals.json',
        'requestCounteragents' => 'http://api.dellin.stage/v1/customers/counteragents.json',
     ...
     * @param string $functionName название метода
     * @param array $data параметры вызова
     ...
     * @param $params массив параметров для получения статуса (как минимум docid)
     * @return bool|mixed
     */
        ...
        if (isset($result->url)) {
            ...
            if (!$cityKladr) { //если функция вызвана без параметров - отдаем все терминалы по всем городам
                ...
                foreach ($arCities as $city) {
                    ...
                    $arTerminalsByCities[$city->name] = $cityData;
                }
