...
class Cli {
	...
	public static function canAutoSet(){
		if(static::isLinux() && static::isExec()) {
			$strPhpFile = Helper::root().'/bitrix/modules/'.WDU_MODULE.'/cli/check_autoset.php';
	 ...
	 */
	public static function getPhpFile($strFile, $strModuleId=false){
		$strModuleId = strlen($strModuleId) ? $strModuleId : WDU_MODULE;
	 ...
	 *	Add cron job
	 */
	public static function addCronTask($mCommand, $strSchedule=null){
		if(static::isLinux()){
	 ...
	 *	Get cron jobs
	 */
	public static function getCronTasks($strModuleId=false){
		$arResult = [];
		if(static::isExec() && static::isLinux()) {
			$strCommand = 'crontab -l;';
			@exec($strCommand, $arCommandResult);
			$strPath = strlen($strModuleId) ? '/bitrix/modules/'.$strModuleId.'/' : null;
	 ...
	 */
	public static function setProfileCronTasks($intProfileId, $arTasks, $strPhpPath='php', $bMbstring=true, $strConfig='', $strStdout=''){
		...
		foreach($arTasks as $strExternalId => $arTask){
			...
			$arCommand = static::getFullCommand(null, $intProfileId, null, $strExternalId);
	 ...
	 */
	public static function isCronTaskConfigured($strCommand, $strSchedule=null){
		if(!strlen($strCommand) || !static::isExec() || !static::isLinux()) {
			return false;
		}
		$strSchedule = strlen($strSchedule) ? trim($strSchedule) : '';
		...
		foreach(static::getCronTasks() as $arTask){
			if(preg_match('#'.$strPattern.'#i', $arTask['COMMAND'], $arMatch)){
				if(!strlen($strSchedule) || strlen($strSchedule) && $arTask['SCHEDULE'] == $strSchedule){
					return true;
				}
			}
		}
	 ...
	 */
	public static function getFullCommand($strScriptName, $intProfileId=null, $strOutputFilename=null, $strExternalId=null){
		...
		$strStdout = Helper::getOption('php_output_stdout') == 'Y' && !is_null($strOutputFilename) ? $strOutputFilename : null;
