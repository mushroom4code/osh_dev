...
final class CProductQueryBuilder
{
    ...
    //enterego replacement quantity
    public const ENTITY_STORE_PRODUCT = 'STORE_PRODUCT';
    //
	...
	private static function initEntityDescription()
	{
        ...
        //enterego replacement quantity
        $strStorages = implode(',', \Enterego\Subsidiary\Storage::getSiteStorages());
        self::$entityDescription[self::ENTITY_STORE_PRODUCT] = [
            'NAME' => 'b_catalog_store_product',
            'ALIAS' => 'PRDST',
            'JOIN' => "left join (SELECT PRODUCT_ID, SUM(AMOUNT) as QUANTITY, IF (SUM(AMOUNT), 'Y', 'N') as AVAILABLE 
                FROM #NAME# WHERE STORE_ID IN ($strStorages) GROUP BY PRODUCT_ID) #ALIAS# 
                ON #ALIAS#.PRODUCT_ID = #ELEMENT#.ID"
        ];
        //
	}
	...
	private static function initEntityFields()
	{
		...
		self::$entityFields = [
            //enterego replacement quantity
            self::ENTITY_STORE_PRODUCT => self::getStoreProductFields(),
            //
    ...
    /** enterego store product fields for replacement quantity
     * @return array[]
     */
    private static function getStoreProductFields(): array
    {
        return [
            'QUANTITY' => [
                'NAME' => 'QUANTITY',
                ...
                'SELECT_EXPRESSION' => [__CLASS__, 'selectStorageQuantity'],
            ],
            'AVAILABLE' => [
                'NAME' => 'AVAILABLE',
                'ALIAS' => 'AVAILABLE',
	...
	private static function searchFieldEntity(string $field, int $type): ?string
	{
		...
		switch ($type)
		{
			case self::ENTITY_TYPE_FLAT:
                //enterego  replacement quantity
                \Enterego\Subsidiary\Storage::getSiteStorages();
                if (isset(self::$entityFields[self::ENTITY_STORE_PRODUCT][$field])) {
                    return self::ENTITY_STORE_PRODUCT;
                }
                //
	...
	private static function checkPreparedField(?array $field): bool
	{
		...
		if ($field['ENTITY_ID'] === 0)
		{
			if (
                ...
                //enterego replacement quantity
                && $field['ENTITY'] != self::ENTITY_STORE_PRODUCT
                //
     ...
     */
    private static function selectStorageQuantity(array &$parameters, array &$entity, array &$field)
    {
        $field['SELECT'] =  'COALESCE (#FULL_NAME#, 0)';
    }
