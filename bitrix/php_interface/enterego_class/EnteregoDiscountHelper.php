...
class EnteregoDiscountHelper
{
    public function recursiveActionListModifying($array, &$resultArray)
    {
        foreach ($array as $key => $child) {
            if (!empty($child)) {
                if (!str_starts_with($child['CLASS_ID'], 'ActSale') && !str_starts_with($child['CLASS_ID'], 'CondBskt')) {
                    $resultArray['CHILDREN'][] = $child;
                } else {
                    if (str_starts_with($child['CLASS_ID'], 'ActSaleBsktGrp')) {
                        if (isset($child['DATA']['All'])) {
                            $resultArray['DATA']['All'] = $child['DATA']['All'];
                        }
                        if (isset($child['DATA']['True'])) {
                            $resultArray['DATA']['True'] = $child['DATA']['True'];
                        }
                    }
                    $this->recursiveActionListModifying($child['CHILDREN'], $resultArray);
                }
            }
        }
    }
    ...
    public function parseConditionLevel($condition, $params)
    {
        $result = array();
    ...
    public function parseConditionName(array $condition)
    {
        $name = '';
    ...
    public function parseConditionOperator($condition)
    {
        $operator = '';
    ...
    public function parseConditionValue($condition, $name)
    {
    ...
    public function parsePropertyCondition(array &$result, array $condition, $params)
    {
        if (!empty($result))
        {
            $subFilter = array();
