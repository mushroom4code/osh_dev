...
class DiscountCouponTable extends Main\Entity\DataManager
{
    ...
    //enterego
    const TYPE_ONE_FOR_USER = 0x0008;
	...
	public static function getCouponTypes($extendedMode = false)
	{
		...
		if ($extendedMode)
		{
			return array(
                ...
                //enterego
                self::TYPE_ONE_FOR_USER  => Loc::getMessage('DISCOUNT_COUPON_TABLE_TYPE_ONE_FOR_USER'),
			);
		}
        //enterego - add TYPE_ONE_FOR_USER
		return array(self::TYPE_BASKET_ROW, self::TYPE_ONE_ORDER, self::TYPE_MULTI_ORDER, self::TYPE_ONE_FOR_USER);
	}
	...
	public static function saveApplied($coupons, $userId, Main\Type\DateTime $currentTime)
	{
		...
		while ($existCoupon = $couponIterator->fetch())
		{
            ...
            //enterego
			elseif ($existCoupon['TYPE'] == self::TYPE_MULTI_ORDER || $existCoupon['TYPE'] == self::TYPE_ONE_FOR_USER)
			{
	...
	public static function isValidCouponType($couponType)
	{
		...
		return (
            ...
            //enterego
            || $couponType == self::TYPE_ONE_FOR_USER
		);
	}
