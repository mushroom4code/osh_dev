...
use Bitrix\Sale\Exchange\EnteregoImport;
...
if ($_GET["mode"] == "checkauth" && $USER->IsAuthorized()) {
    ...
    if ($_GET["mode"] == "init") {
        ...
        if ($_COOKIE[COption::GetOptionString("sale", "export_session_name_" . $curPage, "")]
            == COption::GetOptionString("sale", "export_session_id_" . $curPage, "")) {
            $_SESSION["BX_CML2_EXPORT"][CSaleExportEe::getOrderPrefix()] = 0;

            COption::SetOptionString("sale", "last_export_time_committed_" . $curPage,
                COption::GetOptionString("sale", "last_export_time_" . $curPage, ""));
    ...
    } elseif ($_GET["mode"] == "file")// new version
    {
        ...
        $result = CSaleExportEe::UnZip($_SESSION["BX_CML2_EXPORT"]["zip"], $_SESSION["BX_CML2_EXPORT"]["last_zip_entry"]);
        if ($result === false) {
        ...
        if (file_exists($ABS_FILE_NAME) && filesize($ABS_FILE_NAME) > 0) {
            ...
            $loader = new \Bitrix\Sale\Exchange\EntCSaleOrderLoader();
            $loader->arParams = $arParams;
...
//            EnteregoHandlers::UpdateStopAccessToSpecialPriceTypesOnSale();
            ...
            if (\Bitrix\Main\Config\Option::get("sale", "~IS_SALE_CRM_SITE_MASTER_FINISH", "N") == "Y" ||
                ...
                $o->registerNodeHandler("/" . GetMessage("CC_BSC1_COM_INFO") . "/" . GetMessage("CC_BSC1_AGENTS") . "/" . GetMessage("CC_BSC1_AGENT"), function (CDataXML $xmlObject) use ($o, $loader) {
                    ImportOneCContragent::configuration();
                    $loader->importer = new ImportOneCContragent();

                    $loader->nodeHandler($xmlObject, $o);
                });
                $o->registerNodeHandler("/" . GetMessage("CC_BSC1_COM_INFO") . "/" . GetMessage("CC_BSC1_COMPANYS") . "/" . GetMessage("CC_BSC1_COMPANY"), function (CDataXML $xmlObject) use ($o, $loader) {
                    EnteregoImport::configuration($xmlObject);
                    $loader->importer = new EnteregoImport();
                    $loader->nodeHandler($xmlObject, $o);
                });
                $o->registerNodeHandler("/" . GetMessage("CC_BSC1_COM_INFO") . "/" . GetMessage("CC_BSC1_USERS") . "/" . GetMessage("CC_BSC1_USER"), function (CDataXML $xmlObject) use ($o, $loader) {
                    EnteregoUser::configuration($xmlObject);
                    $loader->importer = new EnteregoUser();
                    $loader->nodeHandler($xmlObject, $o);
                });
            }
            //endregion
...
if (!$bDesignMode) {
    ...
    <table class="data-table">
        <tr>
            <td><? echo GetMessage("CP_BCI1_SITE_LIST") ?></td>
            <td><? echo $arParams["SITE_LIST"] ?></td>
        </tr>
        <tr>
            <td><? echo GetMessage("CP_BCI1_EXPORT_PAYED_ORDERS") ?></td>
            <td><? echo $arParams["EXPORT_PAYED_ORDERS"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td>
        </tr>
        <tr>
            <td><? echo GetMessage("CP_BCI1_EXPORT_ALLOW_DELIVERY_ORDERS") ?></td>
            <td><? echo $arParams["EXPORT_ALLOW_DELIVERY_ORDERS"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td>
        </tr>
        <tr>
            <td><? echo GetMessage("CP_BCI1_CHANGE_STATUS_FROM_1C") ?></td>
            <td><? echo $arParams["CHANGE_STATUS_FROM_1C"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td>
        </tr>
        <tr>
            <td><? echo GetMessage("CP_BCI1_EXPORT_FINAL_ORDERS") ?></td>
            <td><? echo $arStatuses[$arParams["EXPORT_FINAL_ORDERS"]] ?></td>
        </tr>
        <tr>
            <td><? echo GetMessage("CP_BCI1_FINAL_STATUS_ON_DELIVERY") ?></td>
            <td><? echo $arStatuses[$arParams["FINAL_STATUS_ON_DELIVERY"]] ?></td>
        </tr>
        <tr>
            <td><? echo GetMessage("CP_BCI1_REPLACE_CURRENCY") ?></td>
            <td><? echo $arParams["REPLACE_CURRENCY"] ?></td>
        </tr>
        <tr>
            <td><? echo GetMessage("CP_BCI1_USE_ZIP") ?></td>
            <td><? echo $arParams["USE_ZIP"] ? GetMessage("MAIN_YES") : GetMessage("MAIN_NO") ?></td>
        </tr>
    </table>
    <?
}
